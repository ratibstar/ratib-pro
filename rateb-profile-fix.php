 ...
 * https://rateb.sa/rateb-profile-fix.php
 */
...
function rateb_fix_has(string $path, string $needle): bool
{
    if (!is_file($path)) {
        return false;
    }
...
$chrome = $root . '/includes/rateb-home-public-chrome-top.php';
$sync = $root . '/includes/rateb-home-public-nav-sync.php';
$home = $root . '/pages/home.php';
$htaccess = $root . '/.htaccess';
$patch = $root . '/includes/rateb_html_global_ai_patch.php';
$build = $root . '/public/rateb-build.txt';

$diskV13 = rateb_fix_has($chrome, 'v13-onclick');
$diskHeadLock = rateb_fix_has($sync, 'rateb-profile-head-lock');
$diskHtRedirect = rateb_fix_has($htaccess, 'pages/company-profile\\.php$ /profile');
$diskHtmlPatch = rateb_fix_has($patch, 'rateb_register_public_profile_nav_patch');
...
$liveV13 = stripos($liveHome, 'rateb-profile-nav=v13-onclick') !== false;
$liveStale = stripos($liveHome, '/pages/company-profile.php') !== false;
$liveHeadLock = stripos($liveHome, 'rateb-profile-head-lock') !== false;
...
<table>
    ...
    <tr><td>Disk HTML auto-patch</td><td class="<?php echo $diskHtmlPatch ? 'ok' : 'bad'; ?>"><?php echo $diskHtmlPatch ? 'YES' : 'NO — upload rateb_html_global_ai_patch.php'; ?></td></tr>
...
<div class="box">
    ...
    <code>/home/admin/public_html</code>.
</div>
...
<div class="box">
    ...
    <ol>
        ...
        <li><code>includes/rateb_html_global_ai_patch.php</code> — injects click fix when PHP runs</li>
    </ol>
...
<a class="btn" href="<?php echo htmlspecialchars($base . '/rateb-profile-check.php', ENT_QUOTES, 'UTF-8'); ?>">Full check</a>
</body>
</html>
