<?php
/**
 * EN: Handles API endpoint/business logic in `api/workers/bulk-suspended.php`.
 * AR: يدير منطق واجهات API والعمليات الخلفية في `api/workers/bulk-suspended.php`.
 */
...
try {
    ...
    // Update query - set status to suspended
    $sql = "UPDATE workers SET status = 'suspended' WHERE id IN ($placeholders)";
    
    error_log("BULK SUSPENDED - SQL: $sql");
    error_log("BULK SUSPENDED - IDs: " . implode(',', $workerIds));
    ...
    error_log("BULK SUSPENDED - Affected rows: $affectedRows");
    ...
    error_log("BULK SUSPENDED - Verification: " . json_encode($verifyResults));
    
    echo json_encode([
        'success' => true,
        'message' => "Suspended $affectedRows workers",
        'affected_rows' => $affectedRows,
        'verified' => $verifyResults
    ]);
    
} catch (Exception $e) {
    error_log("BULK SUSPENDED ERROR: " . $e->getMessage());
    echo json_encode([
        'success' => false,
