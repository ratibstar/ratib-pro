<?php
/**
 * EN: Handles API endpoint/business logic in `api/notifications/notifications-api.php`.
 * AR: يدير منطق واجهات API والعمليات الخلفية في `api/notifications/notifications-api.php`.
 */
// EN: Start buffered JSON mode to avoid header corruption and mixed output.
// AR: بدء وضع JSON المخزن مؤقتاً لتجنب تلف الترويسات أو اختلاط المخرجات.
...
// EN: CORS preflight short-circuit for browser compatibility.
// AR: معالجة طلبات CORS التمهيدية مبكراً للتوافق مع المتصفحات.
...
// EN: Ensure broadcast target exists in contacts table (reuse or create).
// AR: ضمان وجود مستهدف البث في جدول جهات الاتصال (إعادة استخدام أو إنشاء جديد).
function ensureContactForBroadcast($pdo, $name, $email, $phone, $type, $city = null, $country = null, $company = '') {
    ...
    if ($existing) {
        // Return existing contact ID - use the name from database, not the passed name
        // This ensures we use the correct name associated with this email
        return [
            'id' => (int)$existing['id'],
            'name' => $existing['name'] // Use the name from the contacts table, not the source
        ];
    }
    ...
    return [
        ...
        'name' => $name ?: 'New Contact' // Use the passed name for new contacts
    ];
}
...
function normalizeEmailForBroadcast($email) {
    ...
    // Remove all whitespace inside email
    $normalized = preg_replace('/\s+/', '', $normalized);
    
    // For Gmail addresses, remove dots before @ (Gmail treats user.name@gmail.com same as username@gmail.com)
    $emailParts = explode('@', $normalized);
    if (count($emailParts) === 2) {
        $domain = $emailParts[1];
        // Check if it's Gmail (including googlemail.com which is also Gmail)
...
function isValidEmailForBroadcast($email) {
    ...
    // Block known test/fake email domains
    $blockedDomains = [
        'example.com', 'example.org', 'example.net', 'test.com', 'test.org',
        'invalid.com', 'fake.com', 'dummy.com', 'none.com', 'noemail.com',
        'bvb.aa', 'dd.jj', 'ss.sa', // From your bounce messages
    ...
    // Block suspicious 2-letter TLDs that are commonly fake
    ...
    // Block domains where subdomain matches TLD (e.g., salman.salman, mm.nn)
    if (count($parts) >= 2) {
        $subdomain = $parts[count($parts) - 2];
        // If subdomain exactly matches TLD, it's fake (e.g., salman.salman)
        if ($subdomain === $tld) {
            return false;
        }
        // If both subdomain and TLD are very short (1-2 chars), likely fake (e.g., m.n, mm.nn)
        if (strlen($subdomain) <= 2 && strlen($tld) <= 2) {
            return false;
        }
    }
    
    // Block very short domains (likely fake, e.g., m.n, mm.nn)
    if (strlen($domain) < 6) {
        return false;
    }
    ...
    // Block if subdomain is too short or looks like random characters
    if (strlen($domainWithoutTLD) <= 2) {
        return false;
    }
    
    // Block emails with obviously fake patterns
    $fakePatterns = [
        '/^test/', '/^fake/', '/^dummy/', '/^none/', '/^noemail/',
        '/\d{10,}@/', // Long numbers (like 1756840604@example.com)
        '/\.\./', // Double dots
        '/@localhost/', '/@127\.0\.0\.1/', '/@\./', // Invalid hosts
        '/@[a-z]{1,2}\.[a-z]{1,2}$/', // Very short domains like @m.n or @mm.nn
        '/@[a-z]+\.(aa|dd|jj|nn|bb|cc|ee|ff|gg|hh|ii|kk|ll|mm|oo|pp|qq|rr|tt|uu|vv|ww|xx|yy|zz)$/i', // Suspicious TLDs
    ];
...
// Try to load PHPMailer if available (for proper Gmail SMTP support)
$phpmailerPaths = [
