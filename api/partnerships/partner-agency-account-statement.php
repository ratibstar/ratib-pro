<?php
/**
 * GET: partner agency account statement from main journal / chart (Ratib Pro GL).
 */
ob_start();
...
function paJeColumnFlags(PDO $conn): array
{
    ...
    try {
        ...
        error_log('partner-agency-account-statement columns: ' . $e->getMessage());
    }
...
/** @return array<string, bool> */
function paJelColumnFlags(PDO $conn): array
{
    ...
    try {
        ...
        error_log('partner-agency-account-statement jel columns: ' . $e->getMessage());
    }

    return $out;
}

function paJePostedSql(array $flags): string
{
    if ($flags['is_posted']) {
        // NULL treated as posted for legacy rows; drafts use is_posted = 0
...
try {
    ...
    if (!$fa) {
        partnerAgencyStmtJson(['success' => false, 'message' => 'Linked financial account not found.'], 404);
    }
    ...
    try {
        ...
        error_log('partner-agency-account-statement tables: ' . $e->getMessage());
    }
    ...
    if ($tableJe && $tableJel) {
        $jeFlags = paJeColumnFlags($conn);
        $jelFlags = paJelColumnFlags($conn);
        $postedSql = paJePostedSql($jeFlags);
    ...
    partnerAgencyStmtJson(['success' => false, 'message' => 'Could not load account statement.'], 500);
}
