<?php
/**
 * Quick check: can this PHP runtime read rateb_site_content the same way pages/home.php does?
 *
 * Security: set env RATEB_SITE_CONTENT_DIAG_SECRET to a long random string, then call:
 *   GET /api/diagnostics/rateb-site-content-status.php?token=YOUR_SECRET
 *
...
$conn = function_exists('rateb_site_content_db') ? rateb_site_content_db() : null;
$dbOk = $conn instanceof mysqli;
$tableReadable = $dbOk && function_exists('rateb_site_content_db_can_read_table')
    ? rateb_site_content_db_can_read_table($conn)
    : false;
$phoneKey = 'home.topbar.phone_display';
$phoneVal = ($dbOk && function_exists('rateb_site_content_fetch_value_by_key'))
    ? rateb_site_content_fetch_value_by_key($conn, $phoneKey)
    : null;

$cachePath = function_exists('rateb_site_content_public_cache_path_for_read')
...
$batch = [];
if ($dbOk && function_exists('rateb_site_content_fetch_key_values')) {
    $batch = rateb_site_content_fetch_key_values($batchKeys);
}
...
if (function_exists('rateb_site_content_home_flat')) {
    $flat = rateb_site_content_home_flat(false);
    if (function_exists('rateb_site_content_home_ensure_header_nav_labels')) {
        rateb_site_content_home_ensure_header_nav_labels($flat);
    }
...
$payload = [
    ...
    'rateb_site_content_db' => $dbOk,
    'rateb_site_content_table_readable' => $tableReadable,
    'env_RATEB_SITE_CONTENT_PUBLIC_SOURCE' => getenv('RATEB_SITE_CONTENT_PUBLIC_SOURCE') !== false
        ? trim((string) getenv('RATEB_SITE_CONTENT_PUBLIC_SOURCE'))
        : '',
    'effective_RATEB_SITE_CONTENT_PUBLIC_SOURCE' => function_exists('rateb_site_content_public_source_resolved')
        ? rateb_site_content_public_source_resolved()
        : '',
    'defined_RATEB_CMS_HOME_PUBLIC_SOURCE' => defined('RATEB_CMS_HOME_PUBLIC_SOURCE') ? (string) RATEB_CMS_HOME_PUBLIC_SOURCE : null,
    'env_RATEB_SITE_CONTENT_SKIP_DISK_JSON_CACHE' => getenv('RATEB_SITE_CONTENT_SKIP_DISK_JSON_CACHE') !== false
        ? trim((string) getenv('RATEB_SITE_CONTENT_SKIP_DISK_JSON_CACHE'))
        : '',
    ...
    /** true when direct SELECT for phone equals rateb_site_content_home_flat() output — if false, deploy/cache/read path mismatch. */
