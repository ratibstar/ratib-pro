 ...
 * https://rateb.sa/pages/rateb-live-chrome-check.php
 */
...
foreach (['chrome' => $chrome, 'nav-sync' => $sync, 'home' => $home] as $label => $path) {
    ...
    echo 'has_go_profile=' . (stripos($body, 'data-rateb-go-profile') !== false ? 'yes' : 'no') . "\n";
    echo 'has_wireProfileLink=' . (stripos($body, 'wireProfileLink') !== false ? 'yes' : 'no') . "\n";
    echo 'has_rateb_nav_sync_profile=' . (stripos($body, 'rateb-nav-sync-profile') !== false ? 'yes' : 'no') . "\n";
    echo "\n";
}
...
if ($live === false) {
    ...
    echo 'live_has_go_profile=' . (stripos($live, 'data-rateb-go-profile') !== false ? 'yes' : 'no') . "\n";
    echo 'live_has_v13_marker=' . (stripos($live, 'rateb-profile-nav=v13') !== false ? 'yes' : 'no') . "\n";
    if (preg_match('/class="rateb-nav__brand-profile"[^>]*>/', $live, $m)) {
