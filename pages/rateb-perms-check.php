 ...
 *
 * Check:  https://rateb.sa/pages/rateb-perms-check.php
 * Fix:    https://rateb.sa/pages/rateb-perms-check.php?fix=1&key=rateb-deploy-sync-2026
 *
 * Expected:
...
if ($doFix && !hash_equals('rateb-deploy-sync-2026', $key)) {
    http_response_code(403);
    echo "Forbidden. Use: ?fix=1&key=rateb-deploy-sync-2026\n";
    exit;
}

$skipDirs = ['.git', 'node_modules', 'vendor', 'Designed'];

function rateb_mode($path)
{
    if (!file_exists($path)) {
        return null;
    }
...
function rateb_should_skip($path, array $skipDirs)
{
...
// --- Home ---
$homeMode = rateb_mode($home);
$homeOk = rateb_home_ok($homeMode);
echo "--- Home directory ---\n";
echo 'path=' . $home . "\n";
echo 'mode=' . ($homeMode ?? 'missing') . ' owner=' . rateb_owner($home) . ' expected=0711|0750+ ' . ($homeOk ? '[OK]' : '[BAD]') . "\n";
if (!$homeOk) {
    $issues++;
...
// --- public_html ---
$rootMode = rateb_mode($root);
$rootOk = $rootMode === '0755' || rateb_dir_ok($rootMode);
echo "--- public_html ---\n";
echo 'path=' . $root . "\n";
echo 'mode=' . ($rootMode ?? 'missing') . ' owner=' . rateb_owner($root) . ' expected=0755 ' . ($rootMode === '0755' ? '[OK]' : ($rootOk ? '[OK-ish]' : '[BAD]')) . "\n";
echo 'readable=' . (is_readable($root) ? 'yes' : 'no') . "\n";
if ($rootMode !== '0755') {
  if (!rateb_dir_ok($rootMode)) {
    $issues++;
  }
...
// --- Root .htaccess ---
$ht = $root . '/.htaccess';
$htMode = rateb_mode($ht);
$htOk = rateb_file_ok($htMode) && is_readable($ht);
echo "--- Root .htaccess ---\n";
echo 'path=' . $ht . "\n";
echo 'exists=' . (is_file($ht) ? 'yes' : 'no') . "\n";
echo 'mode=' . ($htMode ?? 'n/a') . ' owner=' . rateb_owner($ht) . ' expected=0644 ' . ($htOk ? '[OK]' : '[BAD]') . "\n";
...
foreach ($iter as $item) {
    ...
    if (rateb_should_skip($path, $skipDirs)) {
        continue;
    }

    if ($item->isDir()) {
        $scannedDirs++;
        $mode = rateb_mode($path);
        if ($mode === '0644' || !rateb_dir_ok($mode)) {
            $badDirs++;
            $issues++;
...
if ($doFix) {
    ...
    echo "Re-run check: https://{$host}/pages/rateb-perms-check.php\n";
} else {
