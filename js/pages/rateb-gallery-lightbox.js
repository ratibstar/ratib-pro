...
(function () {
    ...
    function collectDefaultSlides() {
        ...
        if (opRoot) {
            out = Array.prototype.slice.call(opRoot.querySelectorAll('[data-rateb-gallery-open]'));
        }
        return out;
    }
    ...
    function init(options) {
        ...
        var lb = document.getElementById('rateb-program-lightbox');
        if (!lb) {
            return;
        }
        ...
        var imgEl = lb.querySelector('.rateb-program-lightbox__img');
        var capEl = lb.querySelector('.rateb-program-lightbox__caption');
        var controlsEl = document.getElementById('rateb-program-lightbox-controls');
        var counterEl = document.getElementById('rateb-program-lightbox-counter');
        var lbIndex = 0;
        ...
        function openLbAt(i) {
            applyLbIndex(i);
            lb.hidden = false;
            lb.classList.add('rateb-program-lightbox--open');
            document.body.classList.add('rateb-program-lightbox-open');
            syncLbNav();
        }

        function closeLb() {
            lb.hidden = true;
            lb.classList.remove('rateb-program-lightbox--open');
            document.body.classList.remove('rateb-program-lightbox-open');
            if (imgEl) {
        ...
        document.addEventListener('click', function (ev) {
            var closeHit = ev.target.closest('[data-rateb-program-lightbox-close]');
            ...
            if (marqueeGuard) {
                var vpMarq = ev.target.closest('.rateb-program-marquee__viewport');
                if (vpMarq && vpMarq.getAttribute('data-rateb-marquee-suppress-click')) {
                    ev.preventDefault();
                    ev.stopPropagation();
                    vpMarq.removeAttribute('data-rateb-marquee-suppress-click');
                    return;
                }
            }

            if (!lb.hidden) {
                if (ev.target.closest('[data-rateb-program-lightbox-prev]')) {
                    ev.preventDefault();
                    openLbAt(lbIndex - 1);
                    return;
                }
                if (ev.target.closest('[data-rateb-program-lightbox-next]')) {
                    ev.preventDefault();
                    openLbAt(lbIndex + 1);
                    return;
                }
            ...
            var btn = ev.target.closest('[data-rateb-gallery-open], [data-rateb-program-open]');
            if (!btn) {
                return;
            }
            ev.preventDefault();
    ...
    window.RATEBGalleryLightbox = { init: init };
})();
