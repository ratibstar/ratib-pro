 ...
 * Config from #rateb-home-bootstrap (JSON) set in home.php.
...
(function () {
    ...
    /* Public nav: includes/rateb-home-public-chrome-top.php + js/pages/rateb-public-nav-ia-fix.js */
    ...
    function parseBootstrap() {
        ...
        var el = document.getElementById('rateb-home-bootstrap');
        ...
        try {
            ...
            console.error('rateb-home-bootstrap parse error', err);
        }
        return d;
    }
    ...
    window.RATEB_CHECKOUT_CURRENCY = HOME.checkoutCurrency;
    window.RATEB_DISPLAY_CHECKOUT_CURRENCY = HOME.displayCheckoutCurrency;
    window.RATEB_DISPLAY_NGENIUS_LABEL = HOME.displayNgeniusLabel;
    window.RATEB_DISPLAY_USD_RATE = HOME.displayUsdRate;
    window.RATEB_USD_TO_SAR = HOME.usdToSar;
    var openRegister = HOME.openRegister;
    var DISPLAY_MULTIPLIER = 2;

    function ratebScrollToPageHash(hash, pushState) {
        if (!hash || hash === '#') {
            return false;
        }
    ...
    document.addEventListener('click', function (ev) {
        ...
        if (href.charAt(0) === '#') {
            if (ratebScrollToPageHash(href, true)) {
                ev.preventDefault();
            }
            return;
        }
        try {
            ...
            if (ratebScrollToPageHash(url.hash, true)) {
                ev.preventDefault();
            }
        } catch (eNav) {
    ...
    function updatePaymentSummary(subtotal, years, planKeyOpt) {
        ...
        var displayCur = (typeof window.RATEB_DISPLAY_CHECKOUT_CURRENCY === 'string' && window.RATEB_DISPLAY_CHECKOUT_CURRENCY)
            ? String(window.RATEB_DISPLAY_CHECKOUT_CURRENCY).toUpperCase() : 'SAR';
        var gatewayCur = (typeof window.RATEB_CHECKOUT_CURRENCY === 'string' && window.RATEB_CHECKOUT_CURRENCY)
            ? String(window.RATEB_CHECKOUT_CURRENCY).toUpperCase() : 'SAR';
        if (gatewayCur !== 'SAR' && gatewayCur !== 'USD') {
            gatewayCur = 'SAR';
        }
        var usdToDisplay = (typeof window.RATEB_DISPLAY_USD_RATE === 'number' && isFinite(window.RATEB_DISPLAY_USD_RATE) && window.RATEB_DISPLAY_USD_RATE > 0)
            ? window.RATEB_DISPLAY_USD_RATE : 3.75;
        var usdToGateway = (typeof window.RATEB_USD_TO_SAR === 'number' && isFinite(window.RATEB_USD_TO_SAR) && window.RATEB_USD_TO_SAR > 0)
            ? window.RATEB_USD_TO_SAR : 3.75;
        if (gatewayCur === 'USD') {
            usdToGateway = 1.0;
        }
        ...
        ngeniusNote = '<p class="small mb-0 mt-2 rateb-ngenius-currency-note">Card checkout is charged in <strong>' + displayCur + '</strong>: <strong class="rateb-ngenius-sar-total">' + displayCur + ' ' + displayApprox + '</strong> <span class="rateb-ngenius-rate-note">(USD × ' + displayRateStr + ')</span>.</p>';
        if (displayCur !== gatewayCur) {
            ngeniusNote += '<p class="small text-muted mb-0 mt-1 rateb-ngenius-currency-note">You will complete payment in ' + gatewayCur + '.</p>';
        }
...
(function ratebHomeEnsureProductTourNavTab() {
    ...
    function run() {
        ...
        if (!nav || nav.querySelector('[data-rateb-product-tour-tab]')) {
            return;
        }
        if (!nav.querySelector('.rateb-nav__platform-links')) {
            return;
        }
        ...
        a.className = 'rateb-nav__link rateb-nav__link--product-tour';
        a.setAttribute('data-rateb-product-tour-tab', '1');
        a.setAttribute('data-rateb-product-tour-fallback', '1');
        a.innerHTML =
            '<span class="rateb-nav__icon" aria-hidden="true"><svg class="rateb-nav__glyph" viewBox="0 0 24 24" focusable="false"><use href="#rateb-ng-video"/></svg></span>' +
            '<span class="rateb-nav__label">Tour</span>';
...
/* Legacy nav pill row: handled by js/pages/rateb-public-nav-ia-fix.js (IA v3). */

/** Phase 2: subtle scroll reveal — adds classes only; no DOM/markup changes. */
(function ratebHomeScrollReveal() {
    ...
    var main = document.querySelector('.rateb-main');
    if (!main) {
        return;
    }
    var sel =
        '.rateb-hero, .rateb-section, .pricing-section.rateb-pricing-saas, .video-section.rateb-video, .rateb-final-cta';
    ...
    els.forEach(function (el) {
        el.classList.add('rateb-reveal');
        var r = el.getBoundingClientRect();
        var vh = window.innerHeight || document.documentElement.clientHeight;
        if (r.top < vh * 0.92 && r.bottom > -40) {
            el.classList.add('rateb-reveal-visible');
        }
    });
    html.classList.add('rateb-reveal-init');
    html.classList.add('rateb-reveal-ready');
    var io = new IntersectionObserver(
        function (entries) {
            entries.forEach(function (entry) {
                if (entry.isIntersecting) {
                    entry.target.classList.add('rateb-reveal-visible');
    ...
    els.forEach(function (el) {
        if (!el.classList.contains('rateb-reveal-visible')) {
            io.observe(el);
        }
    });
})();
...
(function ratebHomeProgramStrip() {
    function attachProgramStripDrag(viewport, wrapFn) {
        ...
        viewport.addEventListener('pointerdown', function (ev) {
            ...
            /* Do NOT setPointerCapture here — it retargets the click to the viewport so
               [data-rateb-program-open] never receives the synthetic click. Capture only after drag intent. */
        });
        ...
        viewport.addEventListener('pointermove', function (ev) {
            ...
            if (!dragState.dragged && Math.abs(dx) > dragThreshold) {
                ...
                viewport.classList.add('rateb-program-marquee__viewport--dragging');
            }
        ...
        function endDrag(ev) {
            ...
            viewport.classList.remove('rateb-program-marquee__viewport--dragging');
            if (dragState.dragged) {
                viewport.setAttribute('data-rateb-marquee-suppress-click', '1');
            }
            dragState = null;
        }
    ...
    function attachProgramStripScrollButtons(root, viewport, track) {
        ...
        var btnPrev = root.querySelector('[data-rateb-program-marquee-scroll-prev]');
        var btnNext = root.querySelector('[data-rateb-program-marquee-scroll-next]');
        if (!btnPrev || !btnNext) {
            return;
        }
    ...
    function initProgramStripMarqueeAndLightbox() {
        var mqRoot = document.querySelector('[data-rateb-program-marquee]');
        if (mqRoot) {
            var viewport = mqRoot.querySelector('.rateb-program-marquee__viewport');
            var track = mqRoot.querySelector('.rateb-program-marquee__track');
            var mqReduce = window.matchMedia('(prefers-reduced-motion: reduce)');
            if (mqReduce.matches) {
                mqRoot.classList.add('rateb-program-marquee--no-motion');
                if (viewport && track) {
                    function wrapNm() {
                        var w = track.scrollWidth / 2;
                        if (w <= 0) {
                ...
                mqRoot.classList.add('rateb-program-marquee--js-marquee');
                var loopW = 0;
                var tabHidden = false;
        ...
        if (window.RATEBGalleryLightbox && typeof window.RATEBGalleryLightbox.init === 'function') {
            window.RATEBGalleryLightbox.init({
                ...
                collectExtraSlides: function () {
                    var mqForSlides = document.querySelector('[data-rateb-program-marquee]');
                    if (!mqForSlides) {
                        return [];
                    }
                    var allProgOpen = mqForSlides.querySelectorAll('[data-rateb-program-open]');
    ...
    try {
        ...
        if (typeof console !== 'undefined' && console.error) {
            console.error('rateb program strip init failed', eProg);
        }
    }
})();

/** Phase 3: subtle live telemetry (timestamps, sync age, micro jitter) — illustrative only. */
(function ratebHomeLiveTelemetry() {
    ...
    function tickSyncAge() {
        var m = rnd(1, 4);
        var txt = m + 'm';
        document.querySelectorAll('.rateb-live-sync-age').forEach(function (el) {
            el.textContent = txt;
        });
        document.querySelectorAll('.rateb-live-ping').forEach(function (el) {
            el.textContent = txt;
        });
    }

    function tickAgencyCounter() {
        var el = document.getElementById('rateb-topbar-nodes-counter');
        if (!el || !el.classList.contains('rateb-live-counter')) {
            return;
        }
        var base = parseInt(el.getAttribute('data-rateb-counter'), 10);
        if (!isFinite(base)) {
            return;
        }
        var n = base + rnd(-2, 2);
    ...
    function tickIntJitter() {
        document.querySelectorAll('[data-rateb-jitter]').forEach(function (el) {
            var base = parseInt(el.getAttribute('data-rateb-jitter'), 10);
            if (!isFinite(base)) {
                return;
            }
            var n = base + rnd(-3, 3);
    ...
    function tickPctJitter() {
        document.querySelectorAll('[data-rateb-jitter-pct]').forEach(function (el) {
            var base = parseFloat(el.getAttribute('data-rateb-jitter-pct'));
            if (!isFinite(base)) {
                return;
            }
    ...
    function tickEventClock() {
        var els = document.querySelectorAll('.rateb-live-clock');
        if (!els.length) {
            return;
        }
        function pad(x) {
...
(function ratebAboutNavInject() {
    ...
    function fixProfileNav() {
        ...
        document
            .querySelectorAll(
                '.rateb-nav__brand-profile, .rateb-nav__link--about, [data-rateb-profile-nav], .rateb-footer-link--about'
            )
            .forEach(function (a) {
