/**
 * EN: Implements frontend interaction behavior in `js/permissions.js`.
 * AR: ينفذ سلوك تفاعلات الواجهة الأمامية في `js/permissions.js`.
 */
/**
...
window.UserPermissions = {
    ...
    async load() {
        ...
        const isControlProBridge = el && el.getAttribute('data-control-pro-bridge') === '1';
        if (isControlProBridge) {
            this.permissions = ['*'];
            this.isAdmin = true;
            this.loaded = true;
            this.applyPermissions();
            setTimeout(() => this.applyPermissions(), 500);
            return;
        }
        try {
    ...
    has(permission) {
        ...
        const isControl = el && el.getAttribute('data-control') === '1';
        const isControlProBridge = el && el.getAttribute('data-control-pro-bridge') === '1';
        if ((isControl || isControlProBridge) && typeof permission === 'string' && permission.indexOf('hide_dashboard_') === 0) {
