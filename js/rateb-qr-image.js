...
(function (global) {
     ...
     */
    function ratebQrImageUrl(data, size, opts) {
     ...
     */
    function ratebRenderQrImage(host, data, size, opts) {
        if (!host || !data) {
            return;
        }
        var px = Math.max(120, Math.min(512, parseInt(size, 10) || 280));
        ...
        wrap.className = 'rateb-qr-image-wrap';
        var img = document.createElement('img');
        img.className = 'rateb-qr-image';
        img.src = ratebQrImageUrl(data, px, opts);
        img.width = px;
        img.height = px;
    ...
    global.ratebQrImageUrl = ratebQrImageUrl;
    global.ratebRenderQrImage = ratebRenderQrImage;
