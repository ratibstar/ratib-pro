...
- [ ] `SHOW TABLES LIKE 'ratib_infra_%';` includes:
...
- [ ] `RATIB_INFRA_SECRET_KEY` set in server env (32+ char random; not in git)
- [ ] `production-verify.php` → `secret_encryption` = `PASS`
- [ ] Sample: `SELECT encrypted_value FROM ratib_infra_provider_secrets LIMIT 1;` — value is base64 JSON envelope, not readable API key
...
- [ ] `ratib_infra_provider_events` receives `health_check` rows after cron runs
- [ ] Providers UI shows health summary (not ParseError)

## 5. Provider events

- [ ] Trigger a test action (domain search or hosting step) — new rows in `ratib_infra_provider_events`
- [ ] Rows include `request_id`, `duration_ms` where applicable
- [ ] `ratib_infra_audit_entries` still receives orchestration audits (unchanged)
...
- [ ] Log files writable under `/home/outratib/logs/` (or your path)

## 7. DB indexes

- [ ] `SHOW INDEX FROM ratib_infra_provider_events;` includes:
  - `idx_ratib_infra_provider_events_provider`
  - `idx_ratib_infra_provider_events_created`
- [ ] `production-verify.php` → `provider_events_indexes` = `PASS`

## 8. No plaintext secrets

- [ ] No API keys in `ratib_infra_provider_secrets.encrypted_value` (verify sample)
...
- [ ] Optional env tuned if needed:
  - `RATIB_INFRA_HEALTH_LOG_MAX_PER_MINUTE` (default 12)
  - `RATIB_INFRA_FAILURE_LOG_MAX_PER_MINUTE` (default 30)
  - `RATIB_INFRA_EVENTS_RETAIN_HEALTH_DAYS` (default 7)
