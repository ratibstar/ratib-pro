...
Target database: **control panel DB** (`CONTROL_PANEL_DB_NAME`, e.g. `admin_control_panel_db`).

Do **not** run `007_drop_rateb_infra_from_admin_out.sql` on the control panel database.
...
| `004_state_machine_normalization.sql` | **Low–medium** | `UPDATE` on `rateb_infra_provisioning_jobs` — brief row locks; run off-peak if table is large. |
| Full bundle `ALL_for_admin_control_panel_db.sql` | **Low** | Same as above; safe for empty or partially-migrated DBs. |
...
4. `004_state_machine_normalization.sql` *(only if `rateb_infra_provisioning_jobs` already exists)*
...
```bash
export RATEB_INFRA_MYSQL_HOST=127.0.0.1
export RATEB_INFRA_MYSQL_PORT=3306
export RATEB_INFRA_MYSQL_USER=your_mysql_user
export RATEB_INFRA_MYSQL_PASS=your_mysql_password
export RATEB_INFRA_MYSQL_DB=admin_control_panel_db
...
```bash
mysql -h127.0.0.1 -uUSER -p admin_control_panel_db \
  < modules/infrastructure-marketplace/Migrations/ALL_for_admin_control_panel_db.sql
```

### Option C — phpMyAdmin

Import `ALL_for_admin_control_panel_db.sql` into the control panel database only.

## Post-migration verify

1. Create `config/infra.secrets.php` from `config/infra.secrets.example.php` and set `RATEB_INFRA_SECRET_KEY` (32+ byte random hex).
...
Expect `overall: PASS` when `RATEB_INFRA_SECRET_KEY` is set and provider credentials validate.
...
3. **Optional table rollback** — only if you must remove new tables and accept data loss:
   - `DROP TABLE IF EXISTS rateb_infra_provider_events;`
   - `DROP TABLE IF EXISTS rateb_infra_provider_secrets;`
   - Do **not** drop `rateb_infra_audit_entries` or `rateb_infra_provider_activations` if production activations exist.
