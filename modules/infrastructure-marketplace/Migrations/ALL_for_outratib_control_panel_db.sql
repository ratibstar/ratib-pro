...
-- Target (example): outratib_control_panel_db
-- =============================================================================
--
-- To remove duplicate ratib_infra_* from the main app DB after moving here:
--   007_drop_ratib_infra_from_outratib_out.sql (edit USE if your main DB name differs).
--
-- (Optional legacy) ALL_for_outratib_out.sql — same DDL on outratib_out; prefer control DB only + 007 drop on main.
--
-- HOW TO RUN (phpMyAdmin or mysql CLI):
--   1. Select database `outratib_control_panel_db` (or change USE below).
--   2. Import / execute this entire file once.
--
-- PHP (DatabaseConnectionFactory): ratib_infra_* use CONTROL_PANEL_DB_NAME by default
-- (outratib_control_panel_db unless overridden). Optional override: RATIB_INFRA_DB_NAME
-- or RATIB_INFRA_DB_DSN for a different schema / credentials.
--
...
CREATE TABLE IF NOT EXISTS `ratib_infra_worker_heartbeats` (
  ...
  KEY `idx_ratib_infra_worker_heartbeats_heartbeat` (`heartbeat_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `ratib_infra_audit_entries` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_ratib_infra_audit_action` (`action_type`),
  KEY `idx_ratib_infra_audit_tenant` (`tenant_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `ratib_infra_secret_refs` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_ratib_infra_secret_scope_key` (`scope_key`, `secret_key`, `is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `ratib_infra_orders` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `public_id` CHAR(36) NOT NULL,
  `tenant_id` BIGINT UNSIGNED DEFAULT NULL,
  `agency_id` BIGINT UNSIGNED DEFAULT NULL,
  ...
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_ratib_infra_orders_public_id` (`public_id`),
  UNIQUE KEY `uniq_ratib_infra_orders_idempotency` (`idempotency_key`),
  KEY `idx_ratib_infra_orders_scope` (`tenant_id`, `agency_id`, `status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `ratib_infra_services` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `public_id` CHAR(36) NOT NULL,
  `tenant_id` BIGINT UNSIGNED DEFAULT NULL,
  `agency_id` BIGINT UNSIGNED DEFAULT NULL,
  ...
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_ratib_infra_services_public_id` (`public_id`),
  KEY `idx_ratib_infra_services_scope` (`tenant_id`, `agency_id`, `lifecycle_state`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
