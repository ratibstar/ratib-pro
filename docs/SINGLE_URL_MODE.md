...
1. **Login page** – Uses `admin_out` (control DB) to load the country list from `control_countries`.
...
2. **Run Option A migrations** on `admin_out`:
   - `config/migrations/option_a_02_update_control_agencies_all.sql` – Maps each `country_id` to its DB.

3. **Country DBs** – Each country DB (e.g. `admin_bangladesh`, `admin_ethiopia`) must exist and have the app schema.

## Control Panel – Rateb Pro Users

The control panel (e.g. `?control=1`) uses `admin_out` and can manage users across all countries. The “Rateb Pro Users” feature will list users from all country DBs – you can use the Rateb Pro Users page (sidebar) to manage users per country.
...
- **Control panel** (e.g. `?control=1`) may use the main/control DB for cross-country operations; Rateb Pro app logic must always use the country DB for that session.
