...
/**
 * Marketplace Tool Executor — tenant-scoped READ against live ERP tables.
 */
final class MarketplaceToolExecutor
{
    /** @var array<string, array<string, mixed>> */
    private const TOOLS = array (
  'list_marketplace_providers' => 
  array (
    'desc' => 'List marketplace providers (tenant-scoped).',
    'table' => 'rateb_mp_providers',
    'sql' => 'SELECT id, name, status, category_id, rating, created_at
                          FROM rateb_mp_providers WHERE company_id = :cid',
    'search_cols' => 
    array (
      0 => 'name',
    ),
  ...
  array (
    'desc' => 'List marketplace services (tenant-scoped).',
    'table' => 'rateb_mp_services',
    'sql' => 'SELECT id, provider_id, title, status, price, currency, created_at
                          FROM rateb_mp_services WHERE company_id = :cid',
    'search_cols' => 
    array (
      0 => 'title',
    ),
  ...
  array (
    'desc' => 'List marketplace orders (tenant-scoped).',
    'table' => 'rateb_mp_orders',
    'sql' => 'SELECT id, order_no, status, total, currency, provider_id, created_at
                          FROM rateb_mp_orders WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'marketplace_summary' => 
  array (
    'desc' => 'Marketplace summary from live providers/services/orders.',
    ...
    public static function execute(string $toolName, array $arguments, ProcurementAgentContext $ctx): array
    {
        ...
        if ($toolName === 'create_marketplace_order') {
            try {
                return ErpAiWriteTools::createMarketplaceOrder($arguments, $companyId, (int) $ctx->userId);
            } catch (\Throwable $e) {
