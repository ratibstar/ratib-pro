...
/**
 * Shared helpers for Phase 20A EAP domain services.
 * Distinct from legacy WorkflowService / rateb_approval_*.
 */
final class ApprovalSupport
{
    ...
    public static function nextRequestNo(int $companyId): string
    {
        $row = (new EapRequest())->queryOne(
            'SELECT COUNT(*) AS c FROM rateb_eap_requests WHERE company_id = :cid',
            ['cid' => $companyId]
        );
        $n = (int) ($row['c'] ?? 0) + 1;

        return 'APR-' . date('Y') . '-' . str_pad((string) $n, 5, '0', STR_PAD_LEFT);
    }

    public static function nextCode(string $table, string $prefix, int $companyId): string
    {
        $allowed = [
            'rateb_eap_templates', 'rateb_eap_stages', 'rateb_eap_rules', 'rateb_eap_chains',
        ];
    ...
    public static function findRequest(int $id, int $companyId): ?array
    {
        if ($id < 1 || $companyId < 1) {
            return null;
        }
    ...
    /** @return array<string, mixed> */
    public static function assertRequest(int $id, int $companyId): array
    {
        $row = self::findRequest($id, $companyId);
        if ($row === null) {
            throw new \RuntimeException('approval_request_not_found');
        }
