...
/**
 * HR Tool Executor — tenant-scoped READ against live ERP tables.
 */
final class HrToolExecutor
{
  ...
  'get_employee' => 
  array (
    'desc' => 'Get one employee by ID (tenant-scoped).',
    'kind' => 'get',
    'table' => 'rateb_employees',
    'sql' => 'SELECT * FROM rateb_employees WHERE id = :id AND company_id = :cid LIMIT 1',
  ),
  'list_leave_requests' => 
  array (
    'desc' => 'List leave requests (tenant-scoped).',
    'table' => 'rateb_leave_requests',
    'sql' => 'SELECT id, employee_id, leave_type_id, start_date, end_date, days, status, reason, created_at
                          FROM rateb_leave_requests WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'list_hr_departments' => 
  ...
  array (
    'desc' => 'HR workforce summary from live employee/leave tables. Never invent counts.',
    'kind' => 'summary_hr',
  ),
);
