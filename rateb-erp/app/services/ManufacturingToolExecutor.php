...
/**
 * Manufacturing Tool Executor — tenant-scoped READ against live ERP tables.
 */
final class ManufacturingToolExecutor
{
    /** @var array<string, array<string, mixed>> */
    private const TOOLS = array (
  'list_production_orders' => 
  array (
    'desc' => 'List manufacturing production orders (tenant-scoped).',
    'table' => 'rateb_mfg_production_orders',
    'sql' => 'SELECT id, order_no, product_id, status, qty_planned, qty_completed, due_date, created_at
                          FROM rateb_mfg_production_orders WHERE company_id = :cid',
    'search_cols' => 
    array (
      0 => 'order_no',
    ),
  ...
  array (
    'desc' => 'Get one production order by ID.',
    'kind' => 'get',
    'table' => 'rateb_mfg_production_orders',
    'sql' => 'SELECT * FROM rateb_mfg_production_orders WHERE id = :id AND company_id = :cid LIMIT 1',
  ),
  'list_mfg_work_orders' => 
  array (
    'desc' => 'List manufacturing work orders (tenant-scoped).',
    'table' => 'rateb_mfg_work_orders',
    'sql' => 'SELECT id, production_order_id, work_center_id, status, planned_start, planned_end, created_at
                          FROM rateb_mfg_work_orders WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'manufacturing_status_summary' => 
  array (
    'desc' => 'Manufacturing production-order status summary.',
    'kind' => 'group_count',
    'table' => 'rateb_mfg_production_orders',
    'group' => 'status',
  ),
    ...
    public static function execute(string $toolName, array $arguments, ProcurementAgentContext $ctx): array
    {
        ...
        if ($toolName === 'create_production_order') {
            try {
                return ErpAiWriteTools::createProductionOrder($arguments, $companyId, (int) $ctx->userId);
            } catch (\Throwable $e) {
