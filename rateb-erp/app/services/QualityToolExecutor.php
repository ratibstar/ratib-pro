...
/**
 * Quality Tool Executor — tenant-scoped READ against live ERP tables.
 */
final class QualityToolExecutor
{
    /** @var array<string, array<string, mixed>> */
    private const TOOLS = array (
  'list_qms_inspections' => 
  array (
    'desc' => 'List QMS inspections (tenant-scoped).',
    'table' => 'rateb_qms_inspections',
    'sql' => 'SELECT id, inspection_no, status, result, planned_at, completed_at, created_at
                          FROM rateb_qms_inspections WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'list_qms_nonconformities' => 
  array (
    'desc' => 'List QMS nonconformities / NCRs (tenant-scoped).',
    'table' => 'rateb_qms_nonconformities',
    'sql' => 'SELECT id, ncr_no, status, severity, title, opened_at, closed_at, created_at
                          FROM rateb_qms_nonconformities WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'list_qms_corrective_actions' => 
  array (
    'desc' => 'List QMS corrective actions (tenant-scoped).',
    'table' => 'rateb_qms_corrective_actions',
    'sql' => 'SELECT id, action_no, ncr_id, status, due_date, completed_at, created_at
                          FROM rateb_qms_corrective_actions WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'quality_summary' => 
  array (
    'desc' => 'Quality summary: open NCRs and inspection counts from live data.',
    ...
    public static function execute(string $toolName, array $arguments, ProcurementAgentContext $ctx): array
    {
        ...
        if ($toolName === 'create_quality_inspection') {
            try {
                return ErpAiWriteTools::createQualityInspection($arguments, $companyId, (int) $ctx->userId);
            } catch (\Throwable $e) {
                return ErpAiDb::fail('tool_exception');
            }
        }
        if ($toolName === 'create_nonconformity') {
            try {
                return ErpAiWriteTools::createNonconformity($arguments, $companyId, (int) $ctx->userId);
            } catch (\Throwable $e) {
                return ErpAiDb::fail('tool_exception');
            }
        }
