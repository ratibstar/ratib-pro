...
/**
 * Payroll Tool Executor — tenant-scoped READ against live ERP tables.
 */
final class PayrollToolExecutor
{
    /** @var array<string, array<string, mixed>> */
    private const TOOLS = array (
  'list_payroll_cycles' => 
  array (
    'desc' => 'List payroll cycles (tenant-scoped).',
    'table' => 'rateb_payroll_cycles',
    'sql' => 'SELECT id, cycle_code, period_start, period_end, status, created_at
                          FROM rateb_payroll_cycles WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'list_payroll_batches' => 
  array (
    'desc' => 'List payroll batches (tenant-scoped).',
    'table' => 'rateb_payroll_batches',
    'sql' => 'SELECT id, cycle_id, batch_no, status, total_net, employee_count, created_at
                          FROM rateb_payroll_batches WHERE company_id = :cid',
    'status_col' => 'status',
    'order' => 'id DESC',
  ),
  'list_payslips' => 
  array (
    'desc' => 'List payslips (tenant-scoped). Optional employee_id.',
    'table' => 'rateb_payroll_payslips',
    'sql' => 'SELECT id, batch_id, employee_id, gross, net, status, created_at
                          FROM rateb_payroll_payslips WHERE company_id = :cid',
    'extra_int' => 
    array (
  ...
  'payroll_run_summary' => 
  array (
    'desc' => 'Payroll run summary from live cycles/batches/payslips.',
    ...
    public static function execute(string $toolName, array $arguments, ProcurementAgentContext $ctx): array
    {
        ...
        if ($toolName === 'create_payroll_cycle') {
            try {
                return ErpAiWriteTools::createPayrollCycle($arguments, $companyId, (int) $ctx->userId);
            } catch (\Throwable $e) {
