# RATEB ERP — Customer guide (Zero-Touch)

1. Download the installer for your computer.  
2. Run it and wait until it finishes.  
3. Double-click **RATEB ERP** on the desktop.  
4. Sign in and work.

Always open **RATEB ERP** at:

https://rateb.sa/rateb-erp/public/admin/

The green / red light shows whether you are connected. The address in the browser stays the same when you go offline — the app continues from the saved (offline) copy on that same site.

If you open `http://127.0.0.1:8088` while online, the app sends you to the cloud URL above (same product). Local PHP is for sync only. To stay on local for debugging, add `?stay_local=1`.

You never need to install PHP, edit settings files, or start services manually.
