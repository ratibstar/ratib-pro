...
namespace Rateb\App\Marketplace;

/**
 * Marketplace Module — Phase 1 foundation (scaffold; commerce logic in later phases).
 * Tenant-scoped services marketplace — distinct from website Theme Marketplace.
 */
final class MarketplaceModule
{
    private const ROOT_REL = '/modules/marketplace';
    ...
    private static function registerAutoload(): void
    {
        ...
        $modelBundle = [
            ...
            'MarketplaceCategory',
            'MarketplaceService',
            'MarketplaceServiceAvailability',
        ...
        spl_autoload_register(static function (string $class) use ($root, $modelBundle): void {
            ...
            if (str_starts_with($relative, 'Models/') && in_array($short, $modelBundle, true)) {
                $bundle = $root . '/app/Models/MarketplaceModels.php';
                if (is_file($bundle)) {
                    require_once $bundle;
                }
