-- Demo operational data for admin oversight screens (English literals — Arabic labels via UI i18n)
SET NAMES utf8mb4;
...
SELECT @cid, 'Demo Medical Supplier', 'SUP-DEMO', 'supplier@demo.rateb.sa', '+966500000001', 'active'
...
SELECT @cid, 'Main Warehouse', 'WH-MAIN', 'Riyadh', 'active'
FROM DUAL WHERE @cid IS NOT NULL
...
SELECT @cid, @wh, 'Medical Gloves Large', 'GLV-L', 'consumables', 120.000, 'box', 45.00, 50.000, DATE_ADD(CURDATE(), INTERVAL 180 DAY), 'active'
...
INSERT INTO rateb_inventory (company_id, warehouse_id, item_name, sku, category, quantity, unit, unit_cost, reorder_level, expiry_date, status)
SELECT @cid, @wh, 'Disinfectant Solution', 'DSF-500', 'consumables', 35.000, 'bottle', 28.50, 40.000, DATE_ADD(CURDATE(), INTERVAL 90 DAY), 'active'
FROM DUAL WHERE @cid IS NOT NULL AND @wh IS NOT NULL
...
SELECT @cid, 'PR-00001', 'Medical consumables purchase request', 'Procurement', 'medium', 'submitted', 5250.00, 'Demo data for admin oversight'
FROM DUAL WHERE @cid IS NOT NULL
...
SELECT @cid, 'PO-00001', @sup, @pr, 'sent', CURDATE(), DATE_ADD(CURDATE(), INTERVAL 14 DAY), 5250.00, 5250.00, 'Demo purchase order'
...
SELECT @cid, @inv1, @wh, 'in', 120.000, 'demo_seed', 'Initial receipt demo'
...
INSERT INTO rateb_stock_movements (company_id, inventory_id, warehouse_id, movement_type, quantity, reference_type, notes)
SELECT @cid, @inv2, @wh, 'out', 5.000, 'demo_seed', 'Demo issue'
...
SELECT @cid, @sup, CURDATE(), 85, 90, 78, 82, 83.75, 'Demo evaluation', 'published'
FROM DUAL WHERE @cid IS NOT NULL AND @sup IS NOT NULL
...
SELECT @cid, 'RFQ-00001', 'RFQ consumables Q2', 'published', DATE_ADD(CURDATE(), INTERVAL 30 DAY), 'Demo quotation comparison'
FROM DUAL WHERE @cid IS NOT NULL
...
SELECT @cid, @rfq, @sup, 'QT-00002', 4980.00, 'under_review', DATE_ADD(CURDATE(), INTERVAL 45 DAY), 'Alternate demo quote'
FROM DUAL WHERE @cid IS NOT NULL AND @rfq IS NOT NULL AND @sup IS NOT NULL
...
SELECT NULL, 'Purchase request approval', 'purchase_request', 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM rateb_approval_workflows WHERE entity_type = 'purchase_request' AND company_id IS NULL LIMIT 1);

INSERT INTO rateb_approval_workflows (company_id, name, entity_type, is_active)
SELECT NULL, 'Purchase order approval', 'purchase_order', 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM rateb_approval_workflows WHERE entity_type = 'purchase_order' AND company_id IS NULL LIMIT 1);
...
SELECT @wf_pr, 1, @role, 'Procurement review'
FROM DUAL WHERE @wf_pr IS NOT NULL AND @role IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM rateb_approval_workflow_steps WHERE workflow_id = @wf_pr LIMIT 1);

INSERT INTO rateb_approval_workflow_steps (workflow_id, step_order, role_id, label)
SELECT @wf_po, 1, @role, 'PO approval'
FROM DUAL WHERE @wf_po IS NOT NULL AND @role IS NOT NULL
