...
SET @sql = IF(@col = 0,
    'ALTER TABLE rateb_attendance_records ADD COLUMN leave_request_id INT UNSIGNED NULL, ADD INDEX idx_att_leave_request (company_id, leave_request_id)',
    'SELECT 1');
...
SET @sql = IF(@col2 = 0,
    'ALTER TABLE rateb_leave_requests ADD COLUMN paid_snapshot TINYINT(1) NULL',
    'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
