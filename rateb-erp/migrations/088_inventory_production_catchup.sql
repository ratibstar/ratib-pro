-- RATEB ERP — inventory + stock movement production catch-up (idempotent, one column at a time)
-- Run on admin_rateb-erp. Safe to re-run.
SET NAMES utf8mb4;
...
SET @col = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'rateb_inventory' AND COLUMN_NAME = 'min_stock');
SET @sql = IF(@col = 0,
    'ALTER TABLE rateb_inventory ADD COLUMN min_stock DECIMAL(12,3) NOT NULL DEFAULT 0.000 AFTER reorder_level',
    'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @col = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'rateb_inventory' AND COLUMN_NAME = 'max_stock');
SET @sql = IF(@col = 0,
    'ALTER TABLE rateb_inventory ADD COLUMN max_stock DECIMAL(12,3) NULL AFTER min_stock',
    'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col = 0,
    'ALTER TABLE rateb_inventory ADD COLUMN production_date DATE NULL AFTER expiry_date',
    'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col = 0,
    'ALTER TABLE rateb_inventory ADD COLUMN document_path VARCHAR(500) NULL AFTER status',
    'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
