...
/**
 * Phase 25B — read-only quality master-data deltas + static catalogs.
 * Gated by offline.quality.masterdata.
 */
final class QualityOfflineMasterDataDirectoryService
{
    ...
    private const ENTITIES = [
        ...
        'quality_checklist_directory' => [
            ...
            'select' => 'id, company_id, branch_id, code, name, name_ar, checklist_type, status',
            'branch_scoped' => true,
        ],
        'quality_standard_directory' => [
            'table' => 'rateb_qms_standards',
            'select' => 'id, company_id, branch_id, code, name, name_ar, standard_ref, status',
            'branch_scoped' => true,
        ],
        'quality_inspection_status_directory' => ['static' => true, 'entity' => QualityWorkflowService::ENTITY_INSPECTION],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isQualityMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (QualityWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
