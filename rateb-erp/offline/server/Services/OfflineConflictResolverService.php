...
final class OfflineConflictResolverService
{
    ...
    /**
     * Phase 23B — Enterprise HRMS conflict policy (additive status-drift + version).
     *
     * @param array<string, mixed> $clientItem
     * @param array<string, mixed>|null $serverItem
     * @return array<string, mixed>
     */
    public function resolveHumanResources(array $clientItem, ?array $serverItem): array
    {
        $base = $this->resolve($clientItem, $serverItem);
        if (($base['action'] ?? '') === 'reject_client') {
            return $base;
        ...
        $serverStatus = strtolower((string) ($serverItem['status'] ?? $serverItem['workflow_status'] ?? ''));
        if (in_array($serverStatus, ['terminated', 'archived', 'cancelled', 'closed', 'completed'], true)
            && isset($clientItem['expected_status'])
            && (string) $clientItem['expected_status'] !== $serverStatus) {
            return [
                'action' => 'reject_client',
    ...
    /**
     * Phase 24B — Enterprise Payroll conflict policy (additive status-drift + version).
     *
     * @param array<string, mixed> $clientItem
     * @param array<string, mixed>|null $serverItem
     * @return array<string, mixed>
     */
    public function resolvePayroll(array $clientItem, ?array $serverItem): array
    {
        $base = $this->resolve($clientItem, $serverItem);
        if (($base['action'] ?? '') === 'reject_client') {
            return $base;
        ...
        $serverStatus = strtolower((string) ($serverItem['status'] ?? $serverItem['workflow_status'] ?? ''));
        if (in_array($serverStatus, ['closed', 'posted', 'approved', 'archived', 'calculated', 'reviewed'], true)
            && isset($clientItem['expected_status'])
            && (string) $clientItem['expected_status'] !== $serverStatus) {
            return [
                'action' => 'reject_client',
    ...
    /**
     * Quality-specific: reject when expected workflow status drifted.
     *
     * @param array<string, mixed> $clientItem
     * @param array<string, mixed>|null $serverItem
     * @return array<string, mixed>
     */
    public function resolveQuality(array $clientItem, ?array $serverItem): array
    {
        $base = $this->resolve($clientItem, $serverItem);
        if (($base['action'] ?? '') === 'reject_client') {
            return $base;
        ...
        $serverStatus = strtolower((string) ($serverItem['status'] ?? $serverItem['workflow_status'] ?? ''));
        if (in_array($serverStatus, ['approved', 'completed', 'verified', 'closed', 'archived'], true)
            && isset($clientItem['expected_status'])
            && (string) $clientItem['expected_status'] !== $serverStatus) {
            return [
                'action' => 'reject_client',
    ...
    /**
     * Documents-specific: reject when expected workflow status drifted.
     *
     * @param array<string, mixed> $clientItem
     * @param array<string, mixed>|null $serverItem
     * @return array<string, mixed>
     */
    public function resolveDocuments(array $clientItem, ?array $serverItem): array
    {
        $base = $this->resolve($clientItem, $serverItem);
        if (($base['action'] ?? '') === 'reject_client') {
            return $base;
    ...
    /**
     * BI-specific: reject when expected workflow status drifted.
     *
     * @param array<string, mixed> $clientItem
     * @param array<string, mixed>|null $serverItem
     * @return array<string, mixed>
     */
    public function resolveBi(array $clientItem, ?array $serverItem): array
    {
        $base = $this->resolve($clientItem, $serverItem);
        if (($base['action'] ?? '') === 'reject_client') {
            return $base;
