...
/**
 * Phase 24B — read-only payroll master-data deltas + static catalogs.
 * Gated by offline.payroll.masterdata.
 */
final class PayrollOfflineMasterDataDirectoryService
{
    ...
    private const ENTITIES = [
        'payroll_structure_directory' => [
            ...
            'select' => 'id, company_id, branch_id, code, name, name_ar, currency_code, status',
            'branch_scoped' => true,
        ],
        'payroll_cycle_directory' => [
            'table' => 'rateb_payroll_cycles',
            'select' => 'id, company_id, branch_id, code, name, name_ar, frequency, status',
            'branch_scoped' => true,
        ],
        'payroll_batch_status_directory' => ['static' => true, 'entity' => PayrollWorkflowService::ENTITY_BATCH],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isPayrollMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (PayrollWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
