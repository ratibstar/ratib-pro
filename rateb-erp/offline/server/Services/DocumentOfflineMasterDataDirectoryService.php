...
/**
 * Phase 26B — read-only documents master-data deltas + static catalogs.
 * Gated by offline.documents.masterdata.
 */
final class DocumentOfflineMasterDataDirectoryService
{
    /** @var array<string, array{table: string, select: string, branch_scoped: bool}|array{static: true, entity?: string}> */
    private const ENTITIES = [
        'documents_repository_directory' => [
            'table' => 'rateb_dms_repositories',
            'select' => 'id, company_id, branch_id, code, name, name_ar, storage_driver, status',
            'branch_scoped' => true,
        ],
        'documents_category_directory' => [
            'table' => 'rateb_dms_categories',
            'select' => 'id, company_id, branch_id, code, name, name_ar, color, status',
            'branch_scoped' => true,
        ],
        'documents_workflow_status_directory' => ['static' => true, 'entity' => DocumentWorkflowService::ENTITY_DOCUMENT],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isDocumentsMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (DocumentWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
