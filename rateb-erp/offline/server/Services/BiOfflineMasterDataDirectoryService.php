...
/**
 * Phase 27B — read-only BI master-data deltas + static catalogs.
 * Gated by offline.bi.masterdata.
 */
final class BiOfflineMasterDataDirectoryService
{
    /** @var array<string, array{table: string, select: string, branch_scoped: bool}|array{static: true, entity?: string}> */
    private const ENTITIES = [
        'bi_dashboard_directory' => [
            'table' => 'rateb_bi_dashboards',
            'select' => 'id, company_id, branch_id, code, name, name_ar, dashboard_type, workflow_status, status',
            'branch_scoped' => true,
        ],
        'bi_kpi_directory' => [
            'table' => 'rateb_bi_kpis',
            'select' => 'id, company_id, branch_id, code, name, name_ar, metric_key, unit, workflow_status, status',
            'branch_scoped' => true,
        ],
        'bi_workflow_status_directory' => ['static' => true, 'entity' => BusinessIntelligenceWorkflowService::ENTITY_DASHBOARD],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isBiMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (BusinessIntelligenceWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
