...
/**
 * Phase 21B — read-only EPROC master-data deltas + static catalogs.
 * Gated by offline.procurement_enterprise.masterdata.
 */
final class ProcurementEnterpriseOfflineMasterDataDirectoryService
{
    ...
    private const ENTITIES = [
        'eproc_supplier_category_directory' => [
            'table' => 'rateb_eproc_supplier_categories',
            'select' => 'id, company_id, branch_id, code, name, name_ar, parent_id, status',
            'branch_scoped' => true,
        ],
        'eproc_rfq_template_directory' => [
            'table' => 'rateb_eproc_rfq_templates',
            'select' => 'id, company_id, branch_id, code, name, name_ar, default_days, status',
            'branch_scoped' => true,
        ],
        'eproc_tag_directory' => [
            'table' => 'rateb_eproc_tags',
            'select' => 'id, company_id, branch_id, code, name, color, status',
            'branch_scoped' => true,
        ],
        'eproc_supplier_profile_status_directory' => ['static' => true],
        'eproc_tender_status_directory' => ['static' => true],
        'eproc_contract_status_directory' => ['static' => true],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isProcurementEnterpriseMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    {
        $type = match ($entity) {
            'eproc_supplier_profile_status_directory' => ProcurementWorkflowService::ENTITY_SUPPLIER_PROFILE,
            'eproc_tender_status_directory' => ProcurementWorkflowService::ENTITY_TENDER,
            'eproc_contract_status_directory' => ProcurementWorkflowService::ENTITY_CONTRACT,
            default => '',
        };
        $items = [];
        if ($type !== '') {
            foreach (ProcurementWorkflowService::statuses($type) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
