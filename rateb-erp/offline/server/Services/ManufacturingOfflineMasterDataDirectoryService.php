...
/**
 * Phase 22B — read-only MFG master-data deltas + static catalogs.
 * Gated by offline.manufacturing.masterdata.
 */
final class ManufacturingOfflineMasterDataDirectoryService
{
    ...
    private const ENTITIES = [
        'mfg_product_directory' => [
            ...
            'select' => 'id, company_id, branch_id, code, name, name_ar, product_type, workflow_status, status',
            'branch_scoped' => true,
        ],
        'mfg_work_center_directory' => [
            'table' => 'rateb_mfg_work_centers',
            'select' => 'id, company_id, branch_id, code, name, name_ar, capacity_hours_day, status',
            'branch_scoped' => true,
        ],
        'mfg_bom_status_directory' => ['static' => true, 'entity' => ManufacturingWorkflowService::ENTITY_BOM],
        'mfg_production_order_status_directory' => ['static' => true, 'entity' => ManufacturingWorkflowService::ENTITY_PRODUCTION_ORDER],
        'mfg_work_order_status_directory' => ['static' => true, 'entity' => ManufacturingWorkflowService::ENTITY_WORK_ORDER],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isManufacturingMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
    ...
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (ManufacturingWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
