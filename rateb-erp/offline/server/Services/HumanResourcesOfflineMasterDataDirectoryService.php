...
/**
 * Phase 23B — read-only HRMS master-data deltas + static catalogs.
 * Gated by offline.hr.masterdata (isHumanResourcesMasterDataEnabled).
 */
final class HumanResourcesOfflineMasterDataDirectoryService
{
    /** @var array<string, array{table: string, select: string, branch_scoped: bool}|array{static: true, entity?: string}> */
    private const ENTITIES = [
        'hrm_department_directory' => [
            'table' => 'rateb_hrm_departments',
            'select' => 'id, company_id, branch_id, code, name, name_ar, parent_id, status',
            'branch_scoped' => true,
        ],
        'hrm_position_directory' => [
            'table' => 'rateb_hrm_positions',
            'select' => 'id, company_id, branch_id, code, name, name_ar, department_id, status',
            'branch_scoped' => true,
        ],
        'hrm_employee_profile_directory' => [
            'table' => 'rateb_hrm_employee_profiles',
            'select' => 'id, company_id, branch_id, code, first_name, last_name, department_id, position_id, workflow_status, status',
            'branch_scoped' => true,
        ],
        'hrm_employee_status_directory' => ['static' => true, 'entity' => HumanResourcesWorkflowService::ENTITY_EMPLOYEE],
        'hrm_training_status_directory' => ['static' => true, 'entity' => HumanResourcesWorkflowService::ENTITY_TRAINING],
        'hrm_performance_status_directory' => ['static' => true, 'entity' => HumanResourcesWorkflowService::ENTITY_PERFORMANCE],
    ];
    ...
    public function pull(
        ...
        if (!$this->flags()->isHumanResourcesMasterDataEnabled()) {
            return [
                'entity_type' => $entity,
                'items' => [],
        ...
        if (!empty($meta['static'])) {
            return $this->pullStatic($entity, (string) ($meta['entity'] ?? ''));
        }
     ...
     * @return array<string, mixed>
     */
    private function pullStatic(string $entity, string $workflowEntity): array
    {
        $items = [];
        if ($workflowEntity !== '') {
            foreach (HumanResourcesWorkflowService::statuses($workflowEntity) as $i => $code) {
                $items[] = [
                    'id' => $i + 1,
                    'code' => $code,
                    'name' => $code,
