...
<div class="container-fluid py-3">
    ...
    <form method="post" class="border rounded p-3 mb-3 col-lg-7">
        ...
        <div class="row g-2">
            <div class="col-md-5"><input class="form-control" name="subject" placeholder="<?php echo htmlspecialchars(__('subject'), ENT_QUOTES, 'UTF-8'); ?>" required></div>
            <div class="col-md-4"><input class="form-control" type="datetime-local" name="starts_at" required></div>
    ...
    <?php foreach (($items ?? []) as $row): ?>
        <tr><td><?php echo htmlspecialchars((string) ($row['subject'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars((string) ($row['starts_at'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></td><td><?php echo htmlspecialchars(rateb_enum_label((string) ($row['status'] ?? '')), ENT_QUOTES, 'UTF-8'); ?></td></tr>
    <?php endforeach; ?>
