...
$typeLabels = [
    'leads' => __('crm_leads'),
    'contacts' => __('crm_contacts'),
    'companies' => __('crm_companies'),
    'opportunities' => __('crm_opportunities'),
    'quotations' => __('crm_quotations'),
    'activities' => __('crm_activities'),
];
?>
<div class="container-fluid py-3">
    <h1 class="h3 mb-3"><?php echo htmlspecialchars((string) ($title ?? __('crm_unified_search')), ENT_QUOTES, 'UTF-8'); ?></h1>
    <form method="get" class="row g-2 mb-4">
        <div class="col-md-6"><input class="form-control" name="q" value="<?php echo htmlspecialchars((string) ($q ?? ''), ENT_QUOTES, 'UTF-8'); ?>" placeholder="<?php echo htmlspecialchars(__('crm_search_placeholder'), ENT_QUOTES, 'UTF-8'); ?>" autofocus></div>
    ...
    <p class="text-muted"><?php echo htmlspecialchars(__('crm_total'), ENT_QUOTES, 'UTF-8'); ?>: <?php echo (int) ($result['total'] ?? 0); ?></p>
    ...
    <ul class="list-group mb-4">
        ...
        <li class="list-group-item small d-flex justify-content-between">
            <span>
                <?php echo htmlspecialchars(rateb_enum_label((string) ($row['entity_type'] ?? '')), ENT_QUOTES, 'UTF-8'); ?>
            ...
            <span class="text-muted"><?php echo htmlspecialchars(__('crm_relevance'), ENT_QUOTES, 'UTF-8'); ?> <?php echo (int) ($row['relevance'] ?? 0); ?></span>
        </li>
        <?php endforeach; ?>
    </ul>
    <?php endif; ?>
    <?php foreach (['leads','contacts','companies','opportunities','quotations','activities'] as $type): ?>
    <h2 class="h5 mt-3"><?php echo htmlspecialchars((string) ($typeLabels[$type] ?? rateb_enum_label($type)), ENT_QUOTES, 'UTF-8'); ?></h2>
    <?php if (($results[$type] ?? []) === []): ?>
