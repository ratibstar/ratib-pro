        ...
        <div class="table-responsive">
            <table class="table rateb-table mb-0" data-rateb-bulk-table="<?php echo $bulkAny ? '1' : '0'; ?>">
                ...
                <?php } else { foreach ($items as $row) {
                    ...
                    $isDraftManual = $st === 'draft' && ($row['source_type'] ?? '') === 'manual';
                    $canSelect = ($bulkManage && $isDraftManual) || ($bulkApprove && $st === 'draft');
                    ?>
                <tr>
                    <?php if ($bulkAny) { ?>
                    ...
                    <td class="text-nowrap">
                        <a href="<?php echo rateb_app_url('journal-entries/' . (int) $row['id']); ?>" class="btn btn-sm btn-outline-primary"><i class="fas fa-eye"></i></a>
                        <?php if (($canManage ?? false) && $isDraftManual) { ?>
                        <a href="<?php echo rateb_app_url('journal-entries/' . (int) $row['id'] . '/edit'); ?>" class="btn btn-sm btn-outline-secondary"><i class="fas fa-edit"></i></a>
                        <?php } ?>
                    </td>
                </tr>
                <?php } } ?>
