...
<div class="container-fluid py-3">
    ...
    <div class="text-muted mb-3"><?php echo htmlspecialchars((string) ($item['request_no'] ?? ''), ENT_QUOTES, 'UTF-8'); ?> · <?php echo htmlspecialchars((string) ($item['workflow_status'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
    <?php if (!empty($canWorkflow) && ($transitions ?? []) !== []): ?>
    <form method="post" action="<?php echo htmlspecialchars(rateb_url(rateb_app_route('eam/requests') . '/' . (int) $item['id'] . '/transition'), ENT_QUOTES, 'UTF-8'); ?>" class="border rounded p-3 col-lg-6">
