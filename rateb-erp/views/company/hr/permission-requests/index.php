...
Rateb\App\Core\View::partial('hr-nav', ['hrActive' => 'permission-requests']);
?>
    ...
    <div class="rateb-card-body p-0">
        <div class="table-responsive">
            <table class="table rateb-table mb-0">
                <thead>
                <tr>
                    <th><?php echo __('permission_date'); ?></th>
                ...
                <?php } else {
                    foreach ($items as $row) {
                        ...
                        ?>
                <tr>
                    <td><?php echo Rateb\App\Core\View::escape((string) ($row['permission_date'] ?? '')); ?></td>
                    <td><?php echo Rateb\App\Core\View::escape($employeeMap[(string) ($row['employee_id'] ?? '')] ?? '—'); ?></td>
                    <td class="rateb-ltr-num"><?php echo Rateb\App\Core\View::escape(substr((string) ($row['time_from'] ?? ''), 0, 5)); ?></td>
                    <td class="rateb-ltr-num"><?php echo Rateb\App\Core\View::escape(substr((string) ($row['time_to'] ?? ''), 0, 5)); ?></td>
