...
<div class="container-fluid py-3">
    <h1 class="h3 mb-3"><?php echo htmlspecialchars((string) ($title ?? __('project_resources')), ENT_QUOTES, 'UTF-8'); ?></h1>
    ...
    <form method="post" action="<?php echo htmlspecialchars(rateb_url(rateb_app_route('projects/resources')), ENT_QUOTES, 'UTF-8'); ?>" class="border rounded p-3 mb-3">
        <input type="hidden" name="_csrf" value="<?php echo htmlspecialchars(\Rateb\App\Core\Csrf::token(), ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="project_id" value="<?php echo (int) $project_id; ?>">
        <input class="form-control mb-2" name="name" required placeholder="<?php echo htmlspecialchars(__('name'), ENT_QUOTES, 'UTF-8'); ?>">
        <button class="btn btn-primary btn-sm" type="submit"><?php echo htmlspecialchars(__('add'), ENT_QUOTES, 'UTF-8'); ?></button>
    </form>
    <?php endif; ?>
    <ul class="list-group">
        <?php foreach (($items ?? []) as $row): ?>
        <li class="list-group-item"><?php echo htmlspecialchars((string) ($row['name'] ?? ''), ENT_QUOTES, 'UTF-8'); ?> <span class="text-muted small"><?php echo htmlspecialchars((string) ($row['resource_type'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></span></li>
        <?php endforeach; ?>
