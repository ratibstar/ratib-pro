...
<div class="container-fluid py-3">
    <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
        ...
        <a class="btn btn-outline-secondary" href="<?php echo htmlspecialchars(rateb_url(rateb_app_route('hrm/training')), ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(__('back'), ENT_QUOTES, 'UTF-8'); ?></a>
    </div>
    <div class="row g-3">
        <div class="col-lg-8">
            <div class="border rounded p-3 mb-3">
                <div><strong><?php echo htmlspecialchars(__('provider'), ENT_QUOTES, 'UTF-8'); ?>:</strong> <?php echo htmlspecialchars((string) ($item['provider'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
                <div><strong><?php echo htmlspecialchars(__('planned_start'), ENT_QUOTES, 'UTF-8'); ?>:</strong> <?php echo htmlspecialchars((string) ($item['planned_start'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
                <div><strong><?php echo htmlspecialchars(__('planned_end'), ENT_QUOTES, 'UTF-8'); ?>:</strong> <?php echo htmlspecialchars((string) ($item['planned_end'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
                <div><strong><?php echo htmlspecialchars(__('hours'), ENT_QUOTES, 'UTF-8'); ?>:</strong> <?php echo htmlspecialchars((string) ($item['hours'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></div>
                <div class="mt-2"><?php echo nl2br(htmlspecialchars((string) ($item['notes'] ?? ''), ENT_QUOTES, 'UTF-8')); ?></div>
            </div>
            <h2 class="h6"><?php echo htmlspecialchars(__('hrm_enrollments'), ENT_QUOTES, 'UTF-8'); ?></h2>
            <div class="table-responsive border rounded mb-3">
                <table class="table table-sm mb-0">
                    <thead><tr><th>ID</th><th><?php echo htmlspecialchars(__('status'), ENT_QUOTES, 'UTF-8'); ?></th></tr></thead>
                    <tbody>
                    <?php foreach (($enrollments ?? []) as $en): ?>
                        <tr>
                            <td><?php echo (int) ($en['employee_profile_id'] ?? 0); ?></td>
                            <td><?php echo htmlspecialchars((string) ($en['enrollment_status'] ?? $en['status'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (($enrollments ?? []) === []): ?><tr><td colspan="2" class="text-muted"><?php echo htmlspecialchars(__('no_records'), ENT_QUOTES, 'UTF-8'); ?></td></tr><?php endif; ?>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="col-lg-4">
            ...
            <div class="border rounded p-3 mb-3">
                ...
                <form method="post" action="<?php echo htmlspecialchars(rateb_url(rateb_app_route('hrm/training') . '/' . (int) $item['id'] . '/transition'), ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <?php endif; ?>
            <h2 class="h6"><?php echo htmlspecialchars(__('hrm_timeline'), ENT_QUOTES, 'UTF-8'); ?></h2>
