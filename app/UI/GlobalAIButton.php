...
HTML;
    ...
    {
        return <<<'HTML'
<script id="ratebGlobalAiV7">(function(){if(window.__ratebGlobalAiSubmitV7)return;window.__ratebGlobalAiSubmitV7=1;var cfg=document.getElementById('app-config');var base=(cfg&&cfg.getAttribute('data-api-base'))||'';if(!base){var btn=document.getElementById('globalAiActionBtn');var b=(btn&&btn.getAttribute('data-base-url'))||'';base=(b?b.replace(/\/control-panel\/?$/i,''):'')+'/api';}var RUN_URL=base.replace(/\/$/,'')+'/workers/global-ai-run.php';var origFetch=window.fetch;window.fetch=function(url,opts){var u=typeof url==='string'?url:(url&&url.url)||'';if(u.indexOf('worker-onboarding')!==-1){url=typeof url==='string'?RUN_URL:(typeof Request!=='undefined'?new Request(RUN_URL,url):RUN_URL);}return origFetch.call(this,url,opts);};function patchSubmit(){if(!window.GlobalAIAction||window.GlobalAIAction.__ratebGlobalAiV7)return;var legacy=window.GlobalAIAction.submit&&window.GlobalAIAction.submit.bind(window.GlobalAIAction);if(!legacy)return;window.GlobalAIAction.submit=async function(payloadOverride){var payload=payloadOverride;if(!payload&&window.GlobalAIAction.buildPayload){try{payload=window.GlobalAIAction.buildPayload();}catch(e){return legacy(payloadOverride);}}if(!payload||!payload.worker_id)return legacy(payloadOverride);var runBtn=document.getElementById('globalAiRunBtn');if(runBtn){runBtn.disabled=true;runBtn.textContent='Running...';}try{var res=await fetch(RUN_URL,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify(payload)});var data=await res.json().catch(function(){return{};});if(!res.ok||!data.success)return legacy(payloadOverride);if(window.showNotification)window.showNotification('AI workflow completed successfully.','success');return data;}catch(e){return legacy(payloadOverride);}finally{if(runBtn){runBtn.disabled=false;runBtn.textContent='Run AI Workflow';}}};window.GlobalAIAction.__ratebGlobalAiV7=true;}document.addEventListener('DOMContentLoaded',patchSubmit);setTimeout(patchSubmit,400);setTimeout(patchSubmit,2000);})();</script>
HTML;
    }
}
