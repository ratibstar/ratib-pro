...
if (!function_exists('ratib_barcode_login_resolve_connection')) {
    /**
     * @param array<string, mixed> $ctx country_id, agency_id
     */
    function ratib_barcode_login_resolve_connection(array $ctx): ?mysqli
    {
...
if (!function_exists('ratib_barcode_login_build_session')) {
    /**
    ...
    function ratib_barcode_login_build_session(mysqli $loginConn, array $user, array $ctx): ?array
    {
        ...
        try {
            $loginPk = function_exists('ratib_users_primary_key_column')
                ? ratib_users_primary_key_column($loginConn)
                : 'user_id';
