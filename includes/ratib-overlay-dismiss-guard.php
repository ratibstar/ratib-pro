...
if (!function_exists('ratib_emit_overlay_dismiss_guard')) {
    function ratib_emit_overlay_dismiss_guard(): void
    {
        static $done = false;
        if ($done) {
            return;
        ...
        echo '<style id="ratib-overlay-dismiss-guard">';
        ...
        echo '</style>';
        echo '<script id="ratib-overlay-dismiss-guard-js">';
        echo '(function(){function strip(){document.querySelectorAll(';
        echo '".chat-widget-close,.chat-widget-clear,.global-ai-modal-close,[data-ratib-program-lightbox-close]"';
...
ratib_emit_overlay_dismiss_guard();
