 ...
 */
if (!function_exists('rateb_barcode_pair_db')) {
    function rateb_barcode_pair_db(): ?mysqli
    {
        ...
        if (!function_exists('get_control_lookup_conn') && !isset($GLOBALS['conn'])) {
            ...
            if (!isset($GLOBALS['conn']) && defined('DB_HOST') && defined('DB_NAME')) {
                try {
                    ...
                    error_log('rateb_barcode_pair_db connect: ' . $e->getMessage());
                }
            }
        }
        $candidates = [];
        ...
        foreach ($candidates as $db) {
            if (rateb_barcode_pair_ensure_table($db)) {
                return $resolved = $db;
            }
        }
        return null;
...
if (!function_exists('rateb_barcode_pair_ensure_table')) {
    function rateb_barcode_pair_ensure_table(mysqli $db): bool
    {
        static $readyIds = [];
        ...
        try {
            ...
            error_log('rateb_barcode_pair_ensure_table: ' . $e->getMessage());
        }
        return false;
    }
}

if (!function_exists('rateb_barcode_pair_storage_mode')) {
  /**
   * @return 'db'|'file'|null
   */
    function rateb_barcode_pair_storage_mode(): ?string
    {
        static $mode = null;
        if ($mode !== null) {
            return $mode === '' ? null : $mode;
        }
        $dir = rateb_barcode_pair_file_dir();
        if ($dir !== null && is_writable($dir)) {
            $mode = 'file';
            return 'file';
        }
        try {
            $db = rateb_barcode_pair_db();
            if ($db instanceof mysqli && rateb_barcode_pair_ensure_table($db)) {
                $mode = 'db';
                return 'db';
            }
        } catch (Throwable $e) {
            error_log('rateb_barcode_pair_storage_mode db: ' . $e->getMessage());
        }
        $mode = '';
        return null;
    }
...
if (!function_exists('rateb_barcode_pair_file_dir')) {
    function rateb_barcode_pair_file_dir(): ?string
    {
        $tmp = sys_get_temp_dir();
        $candidates = [
            ($tmp !== '' ? $tmp . '/rateb_barcode_login_pairs' : ''),
...
if (!function_exists('rateb_barcode_pair_path')) {
    function rateb_barcode_pair_path(string $token): ?string
    {
...
if (!function_exists('rateb_barcode_pair_db_read')) {
    /**
     * @return array<string, mixed>|null
     */
    function rateb_barcode_pair_db_read(mysqli $db, string $token): ?array
    {
        $row = null;
        try {
            $stmt = $db->prepare(
            ...
            error_log('rateb_barcode_pair_db_read: ' . $e->getMessage());
            return null;
        }
        if (!$row) {
            return null;
...
if (!function_exists('rateb_barcode_pair_db_write')) {
    /**
     * @param array<string, mixed> $data
     */
    function rateb_barcode_pair_db_write(mysqli $db, string $token, array $data): bool
    {
        try {
            ...
            error_log('rateb_barcode_pair_db_write: ' . $e->getMessage());
            return false;
        }
    }
}
...
if (!function_exists('rateb_barcode_pair_read')) {
     ...
     * @return array<string, mixed>|null
     */
    function rateb_barcode_pair_read(string $token): ?array
    {
        $mode = rateb_barcode_pair_storage_mode();
        if ($mode === 'db') {
            $db = rateb_barcode_pair_db();
            if ($db instanceof mysqli) {
                return rateb_barcode_pair_db_read($db, $token);
            }
        }
        $path = rateb_barcode_pair_path($token);
...
if (!function_exists('rateb_barcode_pair_write')) {
    /**
     * @param array<string, mixed> $data
     */
    function rateb_barcode_pair_write(string $token, array $data): bool
    {
        $mode = rateb_barcode_pair_storage_mode();
        if ($mode === 'db') {
            $db = rateb_barcode_pair_db();
            if ($db instanceof mysqli) {
                return rateb_barcode_pair_db_write($db, $token, $data);
            }
        }
        $path = rateb_barcode_pair_path($token);
        if ($path === null) {
...
if (!function_exists('rateb_barcode_pair_create')) {
    /**
     * @param array<string, mixed> $context
     * @return array{ok:bool, token?:string, message?:string}
     */
    function rateb_barcode_pair_create(array $context): array
    {
        if (rateb_barcode_pair_storage_mode() === null) {
        ...
        if (!rateb_barcode_pair_write($token, $data)) {
...
if (!function_exists('rateb_barcode_pair_approve')) {
    /**
     * @param array<string, mixed> $sessionPayload
     */
    function rateb_barcode_pair_approve(string $token, array $sessionPayload): bool
    {
        $data = rateb_barcode_pair_read($token);
        ...
        return rateb_barcode_pair_write($token, $data);
    }
}

if (!function_exists('rateb_barcode_pair_poll')) {
    /**
     * @return array{ok:bool, status?:string, message?:string}
     */
    function rateb_barcode_pair_poll(string $token): array
    {
        $data = rateb_barcode_pair_read($token);
        if ($data === null) {
...
if (!function_exists('rateb_barcode_pair_consume_session')) {
    /**
     * @return array<string, mixed>|null
     */
    function rateb_barcode_pair_consume_session(string $token): ?array
    {
        $data = rateb_barcode_pair_read($token);
        if ($data === null) {
            return null;
