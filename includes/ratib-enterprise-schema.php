...
if (!function_exists('ratib_enterprise_schema_organization')) {
     ...
     */
    function ratib_enterprise_schema_organization(string $baseUrl): array
    {
        $url = rtrim($baseUrl, '/') . '/';
...
if (!function_exists('ratib_enterprise_schema_software_application')) {
    /**
     * @return array<string, mixed>
     */
    function ratib_enterprise_schema_software_application(string $baseUrl): array
    {
        return [
            '@context' => 'https://schema.org',
            ...
            'operatingSystem' => 'Web',
            'description' => ratib_brand_category() . ' — ' . ratib_brand_expansion(),
            'url' => rtrim($baseUrl, '/') . '/',
            'provider' => [
                '@type' => 'Organization',
                'name' => 'RATEB',
...
if (!function_exists('ratib_enterprise_schema_breadcrumb')) {
    /**
     * @param list<array{0:string,1:string}> $crumbs name, url
     *
     * @return array<string, mixed>
     */
    function ratib_enterprise_schema_breadcrumb(array $crumbs): array
    {
        $items = [];
...
if (!function_exists('ratib_enterprise_schema_emit')) {
    /**
     * @param list<array<string, mixed>> $graphs
     */
    function ratib_enterprise_schema_emit(array $graphs): void
    {
        if ($graphs === []) {
            return;
        }
