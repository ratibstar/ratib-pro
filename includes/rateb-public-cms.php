<?php
/**
 * Read resolved public-site CMS values (rateb_site_content keys).
 */
declare(strict_types=1);

if (!function_exists('rateb_brand_name')) {
    function rateb_brand_name(): string
    {
        return 'RATEB';
    }
}

if (!function_exists('rateb_brand_expansion')) {
    function rateb_brand_expansion(): string
    {
        return 'Recruitment Automation & Telemetry Enterprise Base';
    }
}

if (!function_exists('rateb_brand_full_title')) {
    function rateb_brand_full_title(): string
    {
        return rateb_brand_name() . ' — ' . rateb_brand_expansion();
    }
}

if (!function_exists('rateb_brand_category')) {
    function rateb_brand_category(): string
    {
        return 'Enterprise Workforce Program Infrastructure';
    }
}

if (!function_exists('rateb_brand_platform_name')) {
    function rateb_brand_platform_name(): string
    {
...
if (!function_exists('rateb_legal_entity_name')) {
    function rateb_legal_entity_name(): string
    {
        return rateb_brand_platform_name();
    }
}

if (!function_exists('rateb_brand_footer_legal_line')) {
    function rateb_brand_footer_legal_line(): string
    {
        return rateb_brand_name();
    }
}

if (!function_exists('rateb_brand_hero_lead')) {
    function rateb_brand_hero_lead(): string
    {
...
if (!function_exists('rateb_public_cms_flat')) {
    /**
     * @return array<string, string>
     */
    function rateb_public_cms_flat(): array
    {
        static $cache = null;
        if (is_array($cache)) {
            return $cache;
        ...
        if (function_exists('rateb_site_content_home_flat')) {
            $cache = rateb_site_content_home_flat();

            return $cache;
        }
        if (function_exists('rateb_site_content_defaults_home')) {
            $cache = rateb_site_content_defaults_home();
...
if (!function_exists('rateb_public_cms')) {
    function rateb_public_cms(string $key, string $default = ''): string
    {
        $flat = rateb_public_cms_flat();
...
if (!function_exists('rateb_public_cms_lines')) {
    /**
     * @param list<string> $defaultLines
     *
     * @return list<string>
     */
    function rateb_public_cms_lines(string $key, array $defaultLines): array
    {
        $raw = rateb_public_cms($key, '');
        if ($raw === '') {
            return $defaultLines;
        }
        $lines = preg_split('/\r\n|\r|\n/', $raw) ?: [];
...
if (!function_exists('rateb_public_resolve_profile_media_rel')) {
     ...
     */
    function rateb_public_resolve_profile_media_rel(string $rel): string
    {
        ...
        $map = [
            'assets/images/about-rateb-command.png' => 'public/cms-bundle-about.png',
            ...
            'public/profile-media/about-rateb-command.png' => 'public/cms-bundle-about.png',
...
if (!function_exists('rateb_public_bundled_asset_url')) {
    /** Absolute URL for a site-relative asset with cache-busting mtime. */
    function rateb_public_bundled_asset_url(string $baseUrl, string $relPath): string
    {
        $rel = rateb_public_resolve_profile_media_rel($relPath);
        $fs = dirname(__DIR__) . '/' . str_replace('/', DIRECTORY_SEPARATOR, $rel);
        if (!function_exists('rateb_site_content_asset_url')) {
            require_once __DIR__ . '/site-content.php';
        }
        if (function_exists('rateb_site_content_asset_url')) {
            return rateb_site_content_asset_url($baseUrl, $rel, $rel, $fs);
        }
...
if (!function_exists('rateb_public_cms_image')) {
    function rateb_public_cms_image(string $baseUrl, string $key, string $fallbackRel): string
    {
        $stored = rateb_public_cms($key, '');
        $fallbackFs = dirname(__DIR__) . '/' . str_replace('/', DIRECTORY_SEPARATOR, ltrim($fallbackRel, '/'));
        if (!function_exists('rateb_site_content_asset_url')) {
            require_once __DIR__ . '/site-content.php';
        }
        if (function_exists('rateb_site_content_asset_url')) {
            return rateb_site_content_asset_url($baseUrl, $stored, ltrim($fallbackRel, '/'), $fallbackFs);
        }
...
if (!function_exists('rateb_public_cms_image_or')) {
    /**
     * Primary CMS image key; if empty, uses secondary key (avoids duplicate uploads).
     */
    function rateb_public_cms_image_or(string $baseUrl, string $primaryKey, string $secondaryKey, string $fallbackRel): string
    {
        $flat = rateb_public_cms_flat();
        $stored = trim((string) ($flat[$primaryKey] ?? ''));
        $legacyGeneric = 'assets/images/about-rateb-command.png';
        $legacyGenericPublic = 'public/cms-bundle-about.png';
        $legacyGenericScmedia = function_exists('rateb_site_content_media_default_token')
            ? rateb_site_content_media_default_token($primaryKey, 'public/cms-bundle-about.png')
            : '';
