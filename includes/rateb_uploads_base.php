 ...
 */
if (!function_exists('rateb_uploads_project_root')) {
    function rateb_uploads_project_root(): string
    {
...
if (!function_exists('rateb_uploads_can_create_tree')) {
    /**
     * True when PHP can create subdirectories under this base (stricter than is_writable alone).
     */
    function rateb_uploads_can_create_tree(string $baseDir): bool
    {
        ...
        $probe = $baseDir . DIRECTORY_SEPARATOR . '.rateb_mk_' . str_replace('.', '', uniqid('', true));
...
if (!function_exists('rateb_uploads_can_create_worker_subtree')) {
    /**
     ...
     */
    function rateb_uploads_can_create_worker_subtree(string $baseDir): bool
    {
        $baseDir = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $baseDir), DIRECTORY_SEPARATOR);
        if (!rateb_uploads_can_create_tree($baseDir)) {
            return false;
        }
        ...
        $probe = $workers . DIRECTORY_SEPARATOR . '.rateb_mk_' . str_replace('.', '', uniqid('', true));
        if (@mkdir($probe, 0777, true)) {
            @rmdir($probe);
...
if (!function_exists('rateb_uploads_effective_marker_file')) {
    function rateb_uploads_effective_marker_file(): string
    {
        return rateb_uploads_project_root() . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . '.rateb_effective_upload_root';
    }
}

if (!function_exists('rateb_uploads_marker_path_allowed')) {
    /**
     * @param string $abs normalized absolute path
     */
    function rateb_uploads_marker_path_allowed(string $abs, string $projectRoot): bool
    {
...
if (!function_exists('rateb_uploads_read_valid_marker')) {
    function rateb_uploads_read_valid_marker(): ?string
    {
        $mf = rateb_uploads_effective_marker_file();
        if (!is_readable($mf)) {
            return null;
        }
        ...
        $projectRoot = rateb_uploads_project_root();
        if (!rateb_uploads_marker_path_allowed($p, $projectRoot)) {
            return null;
        }
        if (!is_dir($p) || !rateb_uploads_can_create_worker_subtree($p)) {
            return null;
        }
        $rp = realpath($p);
...
if (!function_exists('rateb_uploads_write_marker')) {
    function rateb_uploads_write_marker(string $baseDir): void
    {
        $baseDir = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $baseDir), DIRECTORY_SEPARATOR);
        $projectRoot = rateb_uploads_project_root();
        if (!rateb_uploads_marker_path_allowed($baseDir, $projectRoot)) {
            return;
        }
        $mf = rateb_uploads_effective_marker_file();
        $dir = dirname($mf);
...
if (!function_exists('rateb_uploads_candidate_base_dirs')) {
     ...
     * @return list<string>
     */
    function rateb_uploads_candidate_base_dirs(bool $appendTempFallback = true): array
    {
        $projectRoot = rateb_uploads_project_root();
        ...
        if ($parent !== '' && $parent !== '.' && $parent !== $projectRoot) {
            $out[] = $parent . DIRECTORY_SEPARATOR . 'rateb_uploads';
        }
        $out[] = $projectRoot . DIRECTORY_SEPARATOR . 'storage' . DIRECTORY_SEPARATOR . 'rateb_uploads';
        if ($appendTempFallback) {
            $out[] = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, sys_get_temp_dir()), DIRECTORY_SEPARATOR)
                . DIRECTORY_SEPARATOR . 'rateb_uploads_' . substr(md5($projectRoot), 0, 10);
        }
...
if (!function_exists('rateb_uploads_pick_base_for_worker_document')) {
    /**
     * Find a writable root by actually creating workers/{id}/documents/{type}/ on disk.
     * Persists the first working root so rateb_uploads_base_dir() matches for reads/downloads.
     *
     * @throws RuntimeException when no root works
     */
    function rateb_uploads_pick_base_for_worker_document(int $workerId, string $docType): string
    {
        if ($workerId <= 0) {
        ...
        foreach (rateb_uploads_candidate_base_dirs(true) as $b) {
            $ordered[] = $b;
        }
        $seen = [];
        $last = '';
        foreach ($ordered as $base) {
            ...
            try {
                rateb_uploads_ensure_dir($leaf);
                rateb_uploads_write_marker($base);
                $rp = realpath($base);

                return $rp !== false ? $rp : $base;
            } catch (RuntimeException $e) {
                if ($marker !== null && $base === $marker) {
                    @unlink(rateb_uploads_effective_marker_file());
                }
                $last = $e->getMessage();
            }
        }
...
if (!function_exists('rateb_uploads_base_dir')) {
     ...
     */
    function rateb_uploads_base_dir(): string
    {
        if (defined('RATEB_UPLOADS_BASE')) {
            $v = constant('RATEB_UPLOADS_BASE');
            if (is_string($v) && trim($v) !== '') {
                $p = rtrim(str_replace(['/', '\\'], DIRECTORY_SEPARATOR, trim($v)), DIRECTORY_SEPARATOR);
                if (!is_dir($p)) {
                    @mkdir($p, 0777, true);
                }
                if (is_dir($p) && @is_writable($p) && rateb_uploads_can_create_worker_subtree($p)) {
                    $rp = realpath($p);

                    return $rp !== false ? $rp : $p;
                }
                error_log('RATEB_UPLOADS_BASE is set but not usable for worker upload folders: ' . $p . '; using automatic fallback.');
            }
        }
        $env = getenv('RATEB_UPLOADS_BASE');
        if ($env !== false && trim((string) $env) !== '') {
            ...
            error_log('RATEB_UPLOADS_BASE env is set but not usable for worker upload folders: ' . $p . '; using automatic fallback.');
        }

        $marked = rateb_uploads_read_valid_marker();
        if ($marked !== null) {
            return $marked;
        }
        ...
        foreach (rateb_uploads_candidate_base_dirs(true) as $path) {
            if (!is_dir($path)) {
                @mkdir($path, 0777, true);
            }
            if (rateb_uploads_can_create_worker_subtree($path)) {
                $rp = realpath($path);
                $use = $rp !== false ? $rp : $path;
                if ($path !== $default) {
                    error_log('rateb_uploads_base_dir: using automatic candidate at ' . $use);
                }
        ...
        error_log('rateb_uploads_base_dir: no verified writable root; returning project uploads path (may still fail for uploads).');
        $rp = realpath($default);
...
if (!function_exists('rateb_uploads_ensure_dir')) {
    /**
     ...
     */
    function rateb_uploads_ensure_dir(string $absoluteDir): void
    {
