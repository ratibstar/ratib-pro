 ...
 * Extends ratib-qr-login.php; does not replace password/session architecture.
 */
require_once __DIR__ . '/ratib-qr-login.php';

if (!defined('RATIB_QR_TRUST_DAYS')) {
    define('RATIB_QR_TRUST_DAYS', 30);
}

if (!defined('RATIB_QR_PIN_CHALLENGE_TTL')) {
    define('RATIB_QR_PIN_CHALLENGE_TTL', 300);
}

if (!function_exists('ratib_qr_workforce_signing_secret')) {
    function ratib_qr_workforce_signing_secret(): string
    {
        $pepper = defined('DB_PASS') ? (string) DB_PASS : 'ratib';
        $host = (string) ($_SERVER['HTTP_HOST'] ?? 'rateb');
        return hash('sha256', 'ratib_qr_sign|' . $pepper . '|' . $host);
    }
}

if (!function_exists('ratib_qr_login_build_signed_payload')) {
    /** Build RATIBLOGIN:{64hex}.{8hex_sig} — no user data in QR. */
    function ratib_qr_login_build_signed_payload(string $plainHex): string
    {
        $plainHex = preg_replace('/[^a-f0-9]/', '', strtolower($plainHex));
        $sig = substr(hash_hmac('sha256', $plainHex, ratib_qr_workforce_signing_secret()), 0, 8);
        return RATIB_QR_LOGIN_PREFIX . $plainHex . '.' . $sig;
    }
}

if (!function_exists('ratib_qr_login_verify_signed_plain')) {
    function ratib_qr_login_verify_signed_plain(string $plainHex, string $sig): bool
    {
        $plainHex = preg_replace('/[^a-f0-9]/', '', strtolower($plainHex));
        ...
        $expected = substr(hash_hmac('sha256', $plainHex, ratib_qr_workforce_signing_secret()), 0, 8);
        return hash_equals($expected, substr($sig, 0, 8));
    }
}

if (!function_exists('ratib_qr_workforce_ensure_schema')) {
    function ratib_qr_workforce_ensure_schema(mysqli $db): void
    {
...
if (!function_exists('ratib_qr_workforce_user_row')) {
    function ratib_qr_workforce_user_row(mysqli $db, int $userId): ?array
    {
...
if (!function_exists('ratib_qr_workforce_status')) {
    /**
     * @return array<string, mixed>
     */
    function ratib_qr_workforce_status(mysqli $db, int $userId): array
    {
        $row = ratib_qr_workforce_user_row($db, $userId);
        if (!$row) {
        ...
        $devices = ratib_qr_trusted_devices_list($db, $uid);
        return [
            'ok' => true,
            'user_id' => $uid,
            ...
            'audit_recent' => ratib_qr_workforce_recent_audit($db, $uid, 8),
        ];
    }
}

if (!function_exists('ratib_qr_workforce_recent_audit')) {
    function ratib_qr_workforce_recent_audit(mysqli $db, int $userId, int $limit = 10): array
    {
        ratib_qr_login_ensure_schema($db);
...
if (!function_exists('ratib_qr_login_ensure_persistent_token')) {
     ...
     */
    function ratib_qr_login_ensure_persistent_token(mysqli $db, int $userId, bool $forceRegenerate = false): array
    {
        if ($userId <= 0) {
            return ['ok' => false, 'message' => 'Invalid user.'];
        }
        ratib_qr_workforce_ensure_schema($db);
        $status = ratib_qr_workforce_status($db, $userId);
        if (empty($status['ok'])) {
            return $status;
        }
        if (!$forceRegenerate && ($status['qr_status'] ?? '') === 'active') {
            ratib_qr_login_audit($db, 'token_ensure', 'ok', $userId, null, ['persistent' => true]);
            return [
                'ok' => true,
                'qr_payload' => null,
        ...
        return ratib_qr_login_issue_token($db, $userId, 0, true);
    }
}

if (!function_exists('ratib_qr_login_set_enabled')) {
    function ratib_qr_login_set_enabled(mysqli $db, int $userId, bool $enabled): bool
    {
        ratib_qr_workforce_ensure_schema($db);
        $pk = ratib_users_primary_key_for_barcode($db);
        $val = $enabled ? 1 : 0;
        ...
        $stmt->close();
        if ($ok) {
            ratib_qr_login_audit($db, $enabled ? 'qr_enabled' : 'qr_disabled', 'ok', $userId);
        }
        return $ok;
    }
}

if (!function_exists('ratib_qr_pin_set')) {
    function ratib_qr_pin_set(mysqli $db, int $userId, ?string $pin, bool $enabled): array
    {
        if ($userId <= 0) {
            return ['ok' => false, 'message' => 'Invalid user.'];
        }
        ...
        $stmt->close();
        if ($ok) {
            ratib_qr_login_audit($db, 'pin_updated', 'ok', $userId, null, ['enabled' => $enabled]);
        }
        return ['ok' => $ok, 'message' => $ok ? 'PIN settings saved.' : 'Failed.'];
    }
}

if (!function_exists('ratib_qr_pin_verify')) {
    function ratib_qr_pin_verify(mysqli $db, int $userId, string $pin): bool
    {
        if (!ratib_qr_login_rate_limit_ok('pin_' . $userId, 8)) {
            return false;
        }
        $row = ratib_qr_workforce_user_row($db, $userId);
        ...
        if (strlen($pin) !== 4) {
            ratib_qr_login_audit($db, 'pin_fail', 'fail', $userId, null, ['reason' => 'format']);
            return false;
        }
        $ok = password_verify($pin, (string) $row['qr_pin_hash']);
        ratib_qr_login_audit($db, $ok ? 'pin_ok' : 'pin_fail', $ok ? 'ok' : 'fail', $userId);
        return $ok;
    }
}

if (!function_exists('ratib_qr_pin_required_for_user')) {
    function ratib_qr_pin_required_for_user(array $user): bool
    {
...
if (!function_exists('ratib_qr_challenge_create')) {
    function ratib_qr_challenge_create(mysqli $db, int $userId, ?string $pairToken, array $ctx): ?string
    {
        ratib_qr_workforce_ensure_schema($db);
        try {
        ...
        $expires = date('Y-m-d H:i:s', time() + RATIB_QR_PIN_CHALLENGE_TTL);
...
if (!function_exists('ratib_qr_challenge_consume')) {
    /**
     * @return array{ok:bool, user_id?:int, pair_token?:string, context?:array, message?:string}
     */
    function ratib_qr_challenge_consume(mysqli $db, string $challengeToken): array
    {
        ratib_qr_workforce_ensure_schema($db);
...
if (!function_exists('ratib_qr_device_fingerprint_hash')) {
    function ratib_qr_device_fingerprint_hash(string $fingerprint, string $deviceToken = ''): string
    {
        $fp = trim($fingerprint);
        $tok = preg_replace('/[^a-f0-9]/', '', strtolower($deviceToken));
        return hash('sha256', 'ratib_dev|' . ratib_qr_workforce_signing_secret() . '|' . $fp . '|' . $tok);
    }
}

if (!function_exists('ratib_qr_trusted_device_register')) {
    function ratib_qr_trusted_device_register(mysqli $db, int $userId, string $deviceTokenPlain, string $label = ''): bool
    {
        ratib_qr_workforce_ensure_schema($db);
        $row = ratib_qr_workforce_user_row($db, $userId);
        if (!$row) {
        ...
        $devices = ratib_qr_trusted_devices_list($db, $userId, true);
        if (count($devices) >= $limit) {
            ratib_qr_trusted_device_revoke_oldest($db, $userId);
        }
        $hash = ratib_qr_device_fingerprint_hash('', $deviceTokenPlain);
        $meta = ratib_qr_login_client_meta();
        $expires = date('Y-m-d H:i:s', time() + (RATIB_QR_TRUST_DAYS * 86400));
        ...
        $stmt->close();
        if ($ok) {
            ratib_qr_login_audit($db, 'trusted_device_add', 'ok', $userId, null, ['label' => $label]);
        }
        return $ok;
    }
}

if (!function_exists('ratib_qr_trusted_devices_list')) {
    function ratib_qr_trusted_devices_list(mysqli $db, int $userId, bool $activeOnly = false): array
    {
        ratib_qr_workforce_ensure_schema($db);
...
if (!function_exists('ratib_qr_trusted_device_revoke')) {
    function ratib_qr_trusted_device_revoke(mysqli $db, int $userId, int $deviceId): bool
    {
        ratib_qr_workforce_ensure_schema($db);
        ...
        $stmt->close();
        if ($ok) {
            ratib_qr_login_audit($db, 'trusted_device_revoke', 'ok', $userId, null, ['device_id' => $deviceId]);
        }
        return $ok;
    }
}

if (!function_exists('ratib_qr_trusted_device_revoke_oldest')) {
    function ratib_qr_trusted_device_revoke_oldest(mysqli $db, int $userId): void
    {
        $stmt = @$db->prepare(
        ...
        $stmt->close();
        if ($row) {
            ratib_qr_trusted_device_revoke($db, $userId, (int) $row['id']);
        }
    }
}

if (!function_exists('ratib_qr_trusted_device_validate_cookie')) {
    /**
     * @return array{ok:bool, user_id?:int, message?:string}
     */
    function ratib_qr_trusted_device_validate_cookie(mysqli $db, string $deviceTokenPlain): array
    {
        ...
        ratib_qr_workforce_ensure_schema($db);
        $hash = ratib_qr_device_fingerprint_hash('', $tok);
        $stmt = $db->prepare(
        ...
        ratib_qr_login_audit($db, 'trusted_device_use', 'ok', $uid);
        return ['ok' => true, 'user_id' => $uid];
    }
}

if (!function_exists('ratib_qr_login_apply_session')) {
    /**
     * @param array<string, mixed> $session
     */
    function ratib_qr_login_apply_session(array $session): void
    {
...
if (!function_exists('ratib_qr_set_device_cookie')) {
    function ratib_qr_set_device_cookie(string $deviceTokenPlain): void
    {
        $tok = preg_replace('/[^a-f0-9]/', '', strtolower($deviceTokenPlain));
        if (strlen($tok) < 32) {
            return;
        ...
        setcookie('ratib_device', $tok, [
            'expires' => time() + (RATIB_QR_TRUST_DAYS * 86400),
            'path' => '/',
            'secure' => $secure,
            'httponly' => true,
            'samesite' => 'Lax',
...
if (!function_exists('ratib_qr_workforce_metrics_snapshot')) {
    /**
     * @return array<string, int>
     */
    function ratib_qr_workforce_metrics_snapshot(mysqli $db): array
    {
        ratib_qr_login_ensure_schema($db);
        $out = [
            'scans_ok_24h' => 0,
