...
if (!function_exists('rateb_render_brand_full_title')) {
     ...
     */
    function rateb_render_brand_full_title(array $options = []): void
    {
        ...
        $classes = 'rateb-brand-full rateb-brand-full--' . $variant;
        if ($layout === 'nav-stack') {
            $classes .= ' rateb-brand-full--nav-stack';
        }
        if ($extra !== '') {
            $classes .= ' ' . $extra;
        }
        ...
        if ($layout === 'nav-stack') {
            ...
            echo '<span class="rateb-brand-full__head">';
            echo '<span class="rateb-brand-full__rateb" aria-hidden="true">';
            foreach (['R' => 'r', 'A' => 'a', 'T' => 't', 'E' => 'e', 'B' => 'b'] as $letter => $mod) {
                echo '<span class="rateb-brand-letter rateb-brand-letter--' . $mod . '">' . $letter . '</span>';
            }
            echo '</span>';
            echo '<span class="rateb-brand-full__word rateb-brand-full__word--company">Company</span>';
            echo '</span>';
            if ($showTagline) {
                echo '<span class="rateb-brand-full__tagline" aria-hidden="true">';
                echo '<span class="rateb-brand-full__tagline-row rateb-brand-full__tagline-row--a">';
                foreach ($taglineRowA as $i => $word) {
                    $mod = $taglineModsA[$i] ?? ('w' . (string) ($i + 1));
                    echo '<span class="rateb-brand-full__word rateb-brand-full__word--' . htmlspecialchars($mod, ENT_QUOTES, 'UTF-8') . '">';
                    echo htmlspecialchars($word, ENT_QUOTES, 'UTF-8');
                    echo '</span>';
                }
                echo '</span>';
                echo '<span class="rateb-brand-full__tagline-row rateb-brand-full__tagline-row--b">';
