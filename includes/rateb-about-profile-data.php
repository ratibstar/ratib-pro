...
if (!function_exists('rateb_about_profile_config')) {
    /**
     * @return array<string, mixed>
     */
    function rateb_about_profile_config(string $baseUrl): array
    {
        $shot = static function (string $key, string $fallback, string $altKey, string $defaultAlt) use ($baseUrl): array {
            return [
                'src' => rateb_public_cms_image($baseUrl, $key, $fallback),
                'alt' => rateb_public_cms($altKey, $defaultAlt),
            ];
        };

        return [
            'meta' => [
                'title' => rateb_public_cms('profile.meta.title', 'RATEB — Recruitment Automation & Telemetry Enterprise Base'),
                'description' => rateb_public_cms('profile.meta.description', 'RATEB — enterprise workforce program infrastructure. Platform scope, corridors, and operational capabilities for cross-border recruitment programs.'),
            ],
            'platform' => [
                'eyebrow' => rateb_public_cms('profile.platform.eyebrow', 'Platform overview'),
                'title' => rateb_public_cms('profile.platform.title', 'RATEB dashboard'),
                'lead' => rateb_public_cms('profile.platform.lead', 'Recruitment orchestration, workforce tracking, compliance, and finance-grade operations for sending-country agencies and host-market programs—in one multi-tenant platform.'),
            ],
            'what' => [
                'title' => rateb_public_cms('profile.what.title', 'What RATEB is — and what it is not'),
                'sub' => rateb_public_cms('profile.what.sub', 'RATEB is enterprise workforce program infrastructure for regulated, high-volume international recruitment—not a lightweight CRM or worker tracker.'),
            ],
            'sections' => [
                'arch' => [
                    'eyebrow' => rateb_public_cms('profile.section.arch.eyebrow', 'Platform architecture'),
                    'title' => rateb_public_cms('profile.section.arch.title', 'Platform layers'),
                    'sub' => rateb_public_cms('profile.section.arch.sub', 'Seven layers—from user-facing pages to separate agency databases.'),
                ],
                'ops' => [
                    'eyebrow' => rateb_public_cms('profile.section.ops.eyebrow', 'Agency workspace'),
                    'title' => rateb_public_cms('profile.section.ops.title', 'Agency workspace'),
                    'sub' => rateb_public_cms('profile.section.ops.sub', 'Day-to-day screens for agents, workforce records, finance, HR, and partner coordination.'),
                ],
                'tel' => [
                    'eyebrow' => rateb_public_cms('profile.section.tel.eyebrow', 'Field operations'),
                    'title' => rateb_public_cms('profile.section.tel.title', 'GPS tracking'),
                    'sub' => rateb_public_cms('profile.section.tel.sub', 'Field tracking with offline sync, geofence rules, and audit-friendly event history—not a standalone tracking app.'),
                ],
                'gov' => [
                    'eyebrow' => rateb_public_cms('profile.section.gov.eyebrow', 'Policy & oversight'),
                    'title' => rateb_public_cms('profile.section.gov.title', 'Compliance & governance'),
                    'sub' => rateb_public_cms('profile.section.gov.sub', 'Recorded workflow history, labor-oversight tools, and separate agency data for regulated programs.'),
                ],
                'fin' => [
                    'eyebrow' => rateb_public_cms('profile.section.fin.eyebrow', 'Controllers & CFOs'),
                    'title' => rateb_public_cms('profile.section.fin.title', 'Operational finance infrastructure'),
                    'sub' => rateb_public_cms('profile.section.fin.sub', 'Integrated ledger and invoicing—linked to workers and placements, not a bolt-on spreadsheet.'),
                ],
                'cor' => [
                    'eyebrow' => rateb_public_cms('profile.section.cor.eyebrow', 'Multi-corridor fabric'),
                    'title' => rateb_public_cms('profile.section.cor.title', 'Multi-country operations'),
                    'sub' => rateb_public_cms('profile.section.cor.sub', 'Worker-sending markets on one orchestration core—with country-scoped governance and expanding corridors.'),
                ],
            ],
            'company' => [
                ...
                'address' => rateb_public_cms('profile.company.address', 'Riyadh, Saudi Arabia'),
                'website' => rateb_public_cms('public.contact.website', 'https://rateb.sa'),
                'email' => rateb_public_cms('public.contact.email', 'info@rateb.sa'),
                'phone' => rateb_public_cms('public.contact.phone', '+966 599 863 868'),
                'whatsapp' => rateb_public_cms('public.contact.whatsapp', 'https://wa.me/966599863868'),
                'cr_label' => 'Commercial registration',
                'cr_value' => rateb_public_cms('profile.company.cr_value', 'On file · available under NDA for enterprise procurement'),
                'vat_label' => 'VAT',
                'vat_value' => rateb_public_cms('profile.company.vat_value', 'Available on invoice / registration documents'),
                'industry' => rateb_public_cms('profile.company.industry', 'Workforce program software · Cross-border recruitment operations'),
                'employees_band' => rateb_public_cms('profile.company.employees_band', '51–200 (operations & engineering)'),
            ...
            'screenshots' => [
                ...
                'telemetry' => $shot('profile.image.telemetry', 'public/cms-bundle-gov-tracking.png', 'profile.image.telemetry.alt', 'Tracking map — field checkpoints and corridor context'),
                ...
                'control' => $shot('profile.image.control', 'public/cms-bundle-gov-control-v2.png', 'profile.image.control.alt', 'Control panel — multi-country system settings'),
            ...
            'ops_modules' => [
                ...
                ['title' => 'Worker records', 'body' => 'Longitudinal workforce records, documents, Musaned integration, deployment readiness.', 'icon' => 'fa-users'],
                ...
                ['title' => 'Notifications', 'body' => 'Notifications when queues or SLAs need attention.', 'icon' => 'fa-bell'],
            ],
            'telemetry_features' => [
            ...
            'partner_features' => [
                ...
                ['title' => 'REST API v1', 'body' => 'Bearer-token endpoints for workers, workflows, tracking, and notifications.', 'icon' => 'fa-code'],
            ...
            'not_crm' => rateb_public_cms_lines('profile.what.not_crm', [
                'A lightweight CRM with a basic map plugin',
                'Spreadsheet replacement with a login screen',
                'One shared database for every agency',
                'Marketing site with static dashboard screenshots only',
            ]),
            'is_infrastructure' => rateb_public_cms_lines('profile.what.is_infra', [
                'Enterprise workforce program infrastructure',
                'Multi-country agency dashboard',
                'GPS tracking with operational checkpoints',
