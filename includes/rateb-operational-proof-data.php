...
if (!function_exists('rateb_operational_proof_gov_image_url')) {
    function rateb_operational_proof_gov_image_url(string $baseUrl, string $cmsKey, string $fallbackFile): string
    {
        $govFlat = [
        ...
        return rateb_public_cms_image($baseUrl, $cmsKey, $fallbackRel);
    }
}
...
if (!function_exists('rateb_operational_proof_config')) {
     ...
     */
    function rateb_operational_proof_config(string $baseUrl): array
    {
        $diagram = static function (string $cmsKey, string $file) use ($baseUrl): string {
            ...
            return rateb_public_cms_image($baseUrl, $cmsKey, $fb);
        };
        $govImg = static function (string $cmsKey, string $file) use ($baseUrl): string {
            return rateb_operational_proof_gov_image_url($baseUrl, $cmsKey, $file);
        };

        $cfg = [
            'disclaimer' => rateb_public_cms('profile.opproof.disclaimer', 'Screenshots, diagrams, and metrics on this page use sample operational data or illustrative interfaces. They are not live production dashboards, audited statistics, or evidence of universal government integrations.'),
            'section' => [
                'eyebrow' => rateb_public_cms('profile.opproof.eyebrow', 'Operational proof'),
                'title' => rateb_public_cms('profile.opproof.title', 'How teams run programs on RATEB'),
                'sub' => rateb_public_cms('profile.opproof.sub', 'Government oversight, workforce tracking, and mobile mobilization—then agency workspace, diagrams, and reference flows for procurement review.'),
            ],
            'government' => [
                'id' => 'government-oversight',
                'eyebrow' => rateb_public_cms('profile.gov.eyebrow', 'Government & labor oversight'),
                'title' => rateb_public_cms('profile.gov.title', 'Inspections, violations, geospatial telemetry, and worker mobilization'),
                'lead' => rateb_public_cms('profile.gov.lead', 'RATEB includes a government-aligned control surface for labor oversight demonstrations.'),
                'points' => rateb_public_cms_lines('profile.gov.points', [
                ...
                'screenshots' => [
                    [
                        ...
                        'caption' => rateb_public_cms('profile.gov.caption.control', 'Labor monitoring console—violations, blacklist, worker alerts, and inspection tabs in one place.'),
                    ...
                    [
                        ...
                        'caption' => rateb_public_cms('profile.gov.caption.inspections', 'Inspection history with status badges, inspector attribution, and agency-scoped rows.'),
                        'label' => 'Sample operational data',
                    ...
                    [
                        'title' => 'Tracking Map',
                        'caption' => rateb_public_cms('profile.gov.caption.tracking', 'Tracking map—geofences, playback, and filters for tenant, agency, and country.'),
                        'label' => 'Sample operational data',
                    ...
                    [
                        ...
                        'caption' => rateb_public_cms('profile.gov.caption.onboarding', 'QR-based credentials for worker mobile app mobilization.'),
            ...
            'screenshots' => [
                [
                    ...
                    'label' => 'Illustrative interface',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.pipeline', 'profile.image.pipeline', 'public/cms-bundle-pipeline.svg'),
                    'alt' => rateb_public_cms('profile.image.pipeline.alt', 'Sample workforce pipeline board with stages and SLA column'),
                ],
                [
                    'title' => 'Dashboard',
                    'label' => 'Illustrative interface',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.ops', 'profile.image.ops', 'public/cms-bundle-about.png'),
                    'alt' => rateb_public_cms('profile.image.ops.alt', 'Sample agency dashboard with queues and summaries'),
                ],
                [
                    'title' => 'Finance ledger',
                    'label' => 'Illustrative interface',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.finance', 'profile.image.accounting', 'public/cms-bundle-finance.svg'),
                    'alt' => rateb_public_cms('profile.image.accounting.alt', 'Sample ledger and invoicing screen'),
                ],
                [
                    'title' => 'Audit history',
                    'label' => 'Sample operational data',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.audit', 'profile.gov.image.inspections', 'public/cms-bundle-gov-inspections.png'),
                    'alt' => rateb_public_cms('profile.image.control.alt', 'Sample administration screen with settings and history context'),
                ],
                [
                    'title' => 'Tracking Map',
                    'label' => 'Sample operational data',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.map', 'profile.image.telemetry', 'public/cms-bundle-gov-tracking.png'),
                    'alt' => rateb_public_cms('profile.image.telemetry.alt', 'Sample tracking map with checkpoints and corridor context'),
                ],
                [
                    'title' => 'Partner portal',
                    'label' => 'Illustrative interface',
                    'src' => rateb_public_cms_image_or($baseUrl, 'opproof.image.partner', 'profile.image.partners', 'public/cms-bundle-about.png'),
                    'alt' => rateb_public_cms('profile.image.partners.alt', 'Sample partner portal with deployments and documents'),
                ],
            ],
            'workflows' => [
                [
