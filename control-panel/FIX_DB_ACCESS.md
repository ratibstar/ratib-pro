...
If you still get **Access denied for user 'admin_out'@'localhost' to database 'admin_control_panel_db'** after adding the user in cPanel, use a **dedicate MySQL user** for the control panel.

## Step 1: Create a new MySQL user (cPanel)

1. cPanel → **MySQL® Databases**.
2. Under **Add New User**:
   - **Username:** e.g. `cpanel_cp` (cPanel may add a prefix like `admin_`, so the full name becomes `admin_cpanel_cp`).
...
2. **User:** select the user you just created (e.g. `admin_cpanel_cp`).
3. **Database:** select **admin_control_panel_db**.
4. Click **Add**.
...
2. Set the control panel DB user and password. Either change the defaults or set them via the helper at the top (the `$e()` function reads from environment variables first).
   ...
   ```php
   define('DB_USER', $e('CONTROL_DB_USER', 'admin_cpanel_cp'));   // full username as in cPanel
   ...
   ```php
   define('CONTROL_PANEL_DB_NAME', $e('CONTROL_PANEL_DB_NAME', 'admin_control_panel_db'));
   ```
