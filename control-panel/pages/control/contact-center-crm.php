...
$route = trim((string) ($_GET['route'] ?? 'accounts'), '/');
if ($route === '') {
    $route = 'accounts';
}

$allowedRoutes = ['accounts', 'contacts', 'timeline', 'tags', 'documents', 'sync'];
...
if (!control_contact_center_is_installed()) {
    ...
    startControlLayout('CRM', ['css/system-settings.css'], []);
...
require_once __DIR__ . '/../../includes/control/layout-wrapper.php';
startControlLayout('Contact Center — CRM', [$rccAsset('crm-css')], [], ['standalone' => true]);

$tenantId = control_contact_center_resolve_tenant_id();
$crmApiBase = control_contact_center_crm_api_url();
...
include control_contact_center_root_path() . '/views/components/crm-center-embed.php';

endControlLayout([$rccAsset('realtime-js'), $rccAsset('crm-js')]);
